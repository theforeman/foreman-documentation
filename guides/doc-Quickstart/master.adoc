include::common/attributes.adoc[]
include::common/header.adoc[]
:quickstart-installation-guide:
:context: quick-start
:mode: connected
:ProductName: {ProjectServer}
:tabs-sync-option:

= {QuickstartDocTitle}

This guide describes the quickest way to get a {ProjectServer} up and running.
For a full installation guide, see {InstallingServerDocURL}[_{InstallingServerDocTitle}_].

include::common/modules/ref_system-requirements-for-project.adoc[leveloffset=+1]

include::common/modules/proc_configuring-repositories-for-project.adoc[leveloffset=+1]

include::common/modules/proc_running-project-deployment-utility.adoc[leveloffset=+1]

ifndef::orcharhino,satellite[]
include::common/ribbons.adoc[]
endif::[]
