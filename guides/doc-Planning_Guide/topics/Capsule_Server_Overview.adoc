...
There are two sets of features provided by {SmartProxyServer}s. You can use {SmartProxy} to run services required for host management.
...
ifeval::["{build}" != "satellite"]
The link:{BaseURL}installing_satellite_server_from_a_connected_network/preparing-environment-for-satellite-installation#satellite-ports-and-firewalls-requirements_satellite[Ports and Firewalls Requirements] in _{project-installation-guide-title}_ and {InstallingSmartProxyDocURL}capsule-ports-and-firewalls-requirements_capsule[Ports and Firewalls Requirements] in _{smart-proxy-installation-guide-title}_ contain complete instructions for configuring the host-based firewall to open the ports required.
endif::[]
