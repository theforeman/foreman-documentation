...
Audits can be viewed in the {Project} web UI under *Monitor* > *Audits*.
...
*{SmartProxy} ({SmartProxyServer})*:: An additional server that can be used in a {ProjectName} deployment to facilitate content federation and distribution (act as a Pulp node), and to run other localized services (Puppet Master, *DHCP*, *DNS*, *TFTP*, and more).
...
Catalogs are compiled by a Puppet Master from Puppet Manifests and data from Puppet Agents.
...
Includes software packages (RPM files), Puppet Modules, Docker images, or OSTree snapshots.
...
In the {Project} web UI, you can filter images by tag under *Content* > *Docker Tags*.
...
*External Node Classifier*:: A Puppet construct that provides additional data for a Puppet Master to use when configuring hosts.
{ProjectName} acts as an External Node Classifier to Puppet Masters in a {Project} deployment.
...
*Manifest (Subscription Manifest)*:: A mechanism for transferring subscriptions from the Red{nbsp}Hat Customer Portal to {ProjectName}.
...
*OSTree*:: A tool for managing bootable, immutable, versioned file system trees.
{Project} supports mirroring OSTree snapshots as well as distributing them in content views.
...
ifeval::["{build}" == "satellite"]
For more information and examples of usage, see https://access.redhat.com/documentation/en-us/red_hat_satellite/{ProductVersion}/html/puppet_guide/chap-red_hat_satellite-puppet_guide-building_puppet_modules_from_scratch#sect-{Project_Link}-Puppet_Guide-Building_Puppet_Modules_from_Scratch-Examining_the_Anatomy_of_a_Puppet_Module[Examining the Anatomy of a Puppet Module] in the _Puppet Guide_.
endif::[]
+
...
In the {Project} web UI, you can view those jobs under *Monitor* > *Recurring logics*.
...
You can monitor the task status in the {Project} web UI under *Monitor* > *Tasks*.


[[varl-Glossary_of_Terms-Trend]]
*Trend*:: A means of tracking changes in specific parts of {Project} infrastructure.
Configure trends in {Project} web UI under *Monitor* > *Trends*.
