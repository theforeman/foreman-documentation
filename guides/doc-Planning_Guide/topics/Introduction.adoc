...
For more information on content management see the {BaseURL}content_management_guide/[Content Management Guide].
...
The installation program allows users to install and configure Puppet Masters as a part of {ProjectName} {SmartProxyServer}s.
A Puppet module, running on a Puppet Master on the {ProjectName} Server or {Project} {SmartProxyServer}, is also supported by Red{nbsp}Hat.
...
*Candlepin*:: The only supported methods of using Candlepin are through the {ProjectName} web UI, CLI, and API.
...
*Embedded Tomcat Application Server*:: The only supported methods of using the embedded Tomcat application server are through the {ProjectName} web UI, API, and database.
