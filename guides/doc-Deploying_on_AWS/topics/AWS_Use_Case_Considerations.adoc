...
ifeval::["{build}" != "satellite"]
You can use multi-homed {SmartProxies} by configuring {SmartProxies} with a load balancer. For more information, see {BaseURL}configuring_capsules_with_a_load_balancer/index[Configuring {SmartProxies} with a Load Balancer].
endif::[]
