...
With the {oVirt} compute resource, you can use a `cloud-init` template to configure the provisioned virtual machine. Using the `cloud-init` template avoids having to use any special configuration on the network, such as a managed DHCP and TFTP, to finish the installation of the virtual machine. This method does not require {Project} to connect to the provisioned virtual machine using SSH to run the finish script.

.Prerequisites
ifeval::["{build}" == "foreman"]
* You must enable the remote-execution plug-in to use this template.
endif::[]
* Use the following procedure to add an image: xref:Provisioning_Virtual_Machines_in_Red_Hat_Virtualization-Adding_Red_Hat_Virtualization_Images_on_the_Satellite_Server[]. Ensure that you select the *User Data* check box.
// Where does the user install the cloud-init package? 
+
----
{packages-install} cloud-init
----
+
...
. Click the *Locations* tab, and from the *Locations* list, select the location you want to associate with the template.
. Click the *Organizations* tab, and from the *Organization* list, select the organization you want to associate with the template.
. Click *Submit*.
