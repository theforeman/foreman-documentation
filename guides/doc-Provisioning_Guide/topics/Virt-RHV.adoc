...
. On the virtual machine, install `cloud-init` on this virtual machine:
+
[options="nowrap" subs="+quotes,attributes"]
----
...
You can use the xref:Provisioning_Virtual_Machines_in_Red_Hat_Virtualization-Creating_Hosts_on_a_Red_Hat_Virtualization_Server[] to provision hosts using the 'cloud-init` image and template.
