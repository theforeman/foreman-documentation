...
+
* *DHCP*: the DHCP {SmartProxy} manages the assignment of IP addresses by finding the next available IP address starting from the first address of the range skipping all addresses that are reserved. Before assigning an IP address, {SmartProxy} sends an ICMP and TCP ping to perform a brief check whether the IP address is in use. This check will not work for hosts which are turned off, so the DHCP option can be only used for subnets which are in full {Project} control without any hosts created externally. {SmartProxy} DHCP module retains offered IP addresses for short period of time to prevent collisions during concurrent access, therefore temporary "holes" in the IP range can exist.
