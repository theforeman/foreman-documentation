...
If you encounter these problems, check the DNS setup on both {ProjectServer} and {SmartProxy}, specifically the PTR record resolution.
...
There are three methods of using iPXE with {Project}:
...
With {Project}, you can set up PXE over TFTP to download the `ipxe.efi` file. When the file downloads, all communication continues using HTTP. {Project} uses the iPXE provisioning script either to load an operating system installer or the next entry in the boot order.

When an EFI host boots from PXE, the `ipxe.efi` file changes the process to iPXE. BIOS systems use the `undionly.kpxe` file.

You can use this workflow with or without discovery phase. If you use discovery, use the HTTPBoot  {SmartProxy} module to host the {Project} Discovery image to keep the {Project} discovery image close to the managed nodes.

.Security Information

The iPXE binary in {RHEL} is built without security features, for this reason, you can use HTTP, and cannot use HTTPs. All security-related features of iPXE in {RHEL} are not supported. For more information, see https://access.redhat.com/solutions/3483601[Red Hat Enterprise Linux HTTPS support in iPXE].
...
----
+
. Because the feature cannot be enabled using the foreman-installer command, you must edit the `settings.d/httpboot.yml` configuration file to enable:
+
----
# vi /etc/foreman-proxy/settings.d/httpboot.yml
:enabled: true
----
+
. Restart *foreman-proxy* service and refresh features to get the HTTPBoot feature to appear in the list of features.
+
----
# systemctl restart foreman-proxy
----
+
Repeat for each {SmartProxy} that you want to use.
+
