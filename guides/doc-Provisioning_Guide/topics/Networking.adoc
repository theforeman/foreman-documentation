...
When an error occurs during DHCP orchestration, information may not match between {Project} database and DHCP server. To add missing DHCP records from {Project} database to DHCP server, do:
...
To remove unwanted records from DHCP server do:
...
{Project} assumes that all managed DHCP servers do not contain 3rd party records. This command can possibly delete those unexpected records, use with care. All the mentioned commands only preview what would be done, to actually perform the action add `perform=1` argument.
