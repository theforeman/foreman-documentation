...
Use the `satellite-installer` script with the options to configure these services on the {ProjectServer}.

To configure these services on an external {SmartProxyServer}, run `satellite-installer --scenario capsule`.
...
Run `satellite-installer --scenario capsule --help` to view more options related to DHCP, DNS, TFTP, and other {Project} {SmartProxy} services
...
.Considerations when configuring DNS servers

During DNS record creation, {Project} performs conflict DNS lookups to verify that the hostname is not in active use. This check will be performed against one of the following DNS servers:

* system-wide resolver if *Adminster* > *Settings* > *Query local nameservers* is set to true
...
* Subnet nameservers must be reachable from the {ProjectServer}
* Domain name must have a SOA record available from the {ProjectServer}
...
In {ProjectName} {ProductVersion}, you can configure PXELinux to chainboot iPXE and boot using the HTTP protocol, which is faster and more reliable on high latency networks than TFTP.
...
+
[NOTE]
For `http://satellite.example.com/unattended/iPXE`, you can also use a {ProjectName} {SmartProxy} `http://capsule.example.com:8000/unattended/iPXE`.
...
+
[NOTE]
For `http://satellite.example.com/unattended/iPXE`, you can also use a {ProjectName} {SmartProxy} `http://capsule.example.comf:8000/unattended/iPXE`.
You must update the `/etc/dhcp/dhcpd.conf` file after every upgrade.
The content of the `/etc/dhcp/dhcpd.conf` file is case sensitive.
