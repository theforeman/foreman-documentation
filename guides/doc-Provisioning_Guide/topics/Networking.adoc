...
. To prevent an endless loop of chainbooting iPXE firmware, edit the `/etc/dhcp/dhcpd.conf` file to match the following example. If you use an isolated network, use {SmartProxyServer} URL with TCP port 8000, instead of the URL of {ProjectServer}.
