...
When a finish scripts successfully exits with return code 0, {ProjectName} treats that as a success and the host exit build mode. Note there are few finish scripts which are existing the build mode via "call home" HTTP call, these are not used for image-based provisioning but for post configuration of some operating system installers (Debian/Ubuntu, BSD).
