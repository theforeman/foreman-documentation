...
Post-configuration scripts for providers that accept custom data, also known as seed data. This template does not require {Project} to be able to reach the host, the cloud or virtualization platform is responsible for delivering the data to the image.

Ensure that you meet the following requirements to use the `user_data` template:

* You can use the user_data template to provision virtual machines in cloud or virtualised environments only.
* Ensure that the image that you want to provision supports the user_data template.
* On a host, install the software to read the data and set it to start during boot. For example, `cloud-init`, which expects YAML input, or `ignition`, which expects JSON output. To install the software, you can download it over HTTP, or mount a floppy image, or use any other technique.

Some platforms, for example VMWare, utilise its own way and format for seeding images; you cannot use cloud-init YAML input in these cases. There is a special template kind named `cloud-init` to directly integrate with cloud-init client. For more information, see xref:Provisioning_Virtual_Machines_in_VMware_vSphere-Provisioning_with_cloudinit_and_userdata_templates[].

*cloud_init*

Some environments for example, VMWare, either do not support custom data or have their own format of data that limits what can be done during customization. In this case, you can configure a cloud-init client with the `foreman` plug-in, which attempts to download the template directly from {Project} over HTTP or HTTPS. This technique can be used in any environment, preferably virtualized.

Ensure that you meet the following requirements to use the `clod_init` template:

* An image is preconfigured
* A provisioned host is able to reach out {Project} from the IP address that matches host's provisioning interface IP.

Note that clout-init does not work behind NAT.
