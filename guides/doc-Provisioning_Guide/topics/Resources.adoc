...
Post-configuration scripts to execute via SSH connection when the main provisioning process completes. You can use Finishing templates only for imaged-based provisioning in virtual or cloud environments which do not support user_data. Do not confuse an image with a foreman discovery ISO, which is sometimes called a Foreman discovery image. An image in this context is an install image in a virtualized environment for easy deployment.
...
Post-configuration scripts for providers that accept custom data, also known as seed data. You can use user_data only for cloud or virtualized environments. This template can only be used if the image which is being provisioned supports it, a software must be installed and set to start during boot to read the data. A de-facto standard is `cloud-init` which expects YAML input, or `ignition` which expects JSON output. This template does not require {Project} to be able to reach the host, the cloud or virtualization platform is responsible for delivering the data to the image. Various techniques are used like HTTP download or mounting a floppy image.

Some platforms, like VMWare, utilises its own way and format for seeding images and cloud-init YAML input cannot be used in these cases. There is a special template kind named `cloud-init` to directly integrate with cloud-init client. For more information, see xref:Provisioning_Virtual_Machines_in_VMware_vSphere-Provisioning_with_cloudinit_and_userdata_templates[]

*cloud_init*

Some environments either do not support custom data or have its own format which limits what can be done during customization (e.g VMWare). Cloud-init client can be configured with custom plugin called `foreman` which attempts to download the template directly from {Project} over HTTP(s). This technique can be used in any environment, preferably virtualized. An image must be preconfigured and provisioned host must be able to reach out {Project} from the IP address which matches host's provisioning interface IP. Clout-init will not work behind NAT.
