...
{ProjectName} can interact with Microsoft Azure Resource Manager (RM), including creating new virtual machines and controlling their power management states. Only image-based provisioning is supported for creating Azure RM hosts.

[[Provisioning_Virtual_Machines_in_Azure-Prerequisites_for_Azure_Provisioning]]
=== Prerequisites for Azure RM Provisioning
...
Use this procedure to add a Microsoft Azure RM connection to {Project} to be able to add images and provision hosts. Note that you must add a separate compute resource for a separate Azure RM region.
...
Use the `hammer compute-resource create` command to add Azure connection to Satellite.
...
<7> With the `--region` option, enter the Azure region to use. Note that the value for the `--region` option must be in lowercase.

[[Provisioning_Virtual_Machines_in_GCE-Adding_Azure_Images]]
...
. In the {Project} web UI, navigate to *Infrastructure* > *Compute Resources* and click an Azure connection.
...
. Select an Azure connection.
. From the *Resource group* list, select the resource group to provision to. Click *Reload Images, Sizes, vNets* to load information from Azure RM.
...
. Optional: If you the VM to use a premium SSD, select the *Premium OS Disk* check box.
...
. Optional: If you want to run custom scripts when provisioning finishes, in the *Comma separated file URIs* field, enter comma-separated file URIs of scripts to use. {ProjectName} downloads files to the `/var/lib/waagent/custom-script/download/0/` directory on the host.
. Click *Add Interface*.
...
. Create a compute profile to use with the Azure RM compute resource:
+
[options="nowrap" subs="+quotes"]
----
...
--compute-resource _azure_cr_name_ \         <2>
--organization "_Your_Organization_" \       <3>
--location "_Your_Location_" \               <4>
--compute-attributes="resource_group=_resource_group_,vm_size=_Standard_B1s_,username=_azure_user_,password=_azure_password_,platform=Linux" \ <5>
--interface="compute_public_ip=Dynamic,compute_network=mysubnetID,compute_private_ip=false" <6>
----
...
<3> With the `--organization` option, enter an organization to use.
<4> With the `--location` option, enter a location to use.
<5> With the `--compute-attributes` option, specify the following details:
...
<6> With the `--interface` option, specify the following details:
...
. From the *Deploy on* list, select an Azure connection.
