...
=== Creating Hosts with UEFI HTTP Boot Provisioning in IPv4 Network 
...
. On {Project}, enable the `foreman-proxy-http` and `foreman-proxy-httpboot` plug-ins. Disable system checks with the `--disable-system-checks`  and `--skip-checks-i-know-better` options.
+
...
+
For more information about network interfaces, see {BaseURL}/managing_hosts/index#Adding_Network_Interfaces[Adding network interfaces].
. Start the host and set its boot mode to UEFI HTTP.
. Select the *Kickstart default PXEGrub2 HTTP Boot*. Until BZ#[BZ#] is resolved, selecting *Kickstart Default PXEGrub2* option fails the provisioning process. Until BZ#[] is resolved, ignore the *error: unrecognized network address* warnings that are displayed during boot. 
