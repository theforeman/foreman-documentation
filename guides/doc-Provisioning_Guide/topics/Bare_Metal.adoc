...
When performing unattended and PXE-less provisioning, as a security measure, {Project} automatically generates a unique token and adds this token to the installation recipe URL in the PXE configuration file (PXELinux, Grub2).
ifeval::["{build}" == "satellite"]
...
If token generation is disabled, an attacker can spoof client IP address and download installation recipes from {ProjectServer}, including the encrypted root password.
...
ifeval::["{build}" == "satellite"]
[NOTE]
{ProjectServer} version 6.0 up to 6.6 never reassigned IP address. Discovered hosts maintain the same IP address that is reported during host discovery.
endif::[]
