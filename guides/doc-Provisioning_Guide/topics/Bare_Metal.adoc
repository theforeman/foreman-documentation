...
There are four main ways to provision bare metal instances with {ProjectName}:
...
For UEFI systems, use the *PXEGrub2 UEFI* option to enable a TFTP client to download `grub2/grubx64.efi` file, or use the *PXEGrub2 UEFI HTTP* option to enable an UEFI HTTP client to download `grubx64.efi` from {SmartProxy} with HTTP Boot feature.
ifndef::satellite[]
Use SecureBoot options to enable a client to download the `shim.efi` bootstrap bootloader which then loads signed `grubx64.efi`.
