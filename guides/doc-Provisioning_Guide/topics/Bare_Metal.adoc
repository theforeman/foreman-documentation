...
If you use the Katello plug-in, when the installation completes, the host also registers to {ProjectServer} using the activation key and installs the necessary configuration and management tools from the {ProjectName} Tools repository.
endif::[]
...
<<<<<<< HEAD
=======
[[Provisioning_Bare_Metal_Hosts-Configuring_the_Discovery_Service]]
...
>>>>>>> update ifevals to !=satellite for certain scenarios
