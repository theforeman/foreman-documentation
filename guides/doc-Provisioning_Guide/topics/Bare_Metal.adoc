...
* Ensure that you meet the requirements for HTTP booting. For more information, see link:{BaseURL}/planning_for_red_hat_satellite_6/chap-red_hat_satellite-architecture_guide-provisioning_concepts#http_booting_requirements[HTTP Booting Requirements] in _Planning for {Project}_.

.Procedure

. On {Project}, update the `grub2-efi` package to the latest version:
+
...
. Select *Kickstart default PXEGrub2*.

This creates the host entry and the relevant provisioning settings. This also includes creating the necessary directories and files for UEFI booting the bare metal host. When you start the physical host and set its boot mode to UEFI HTTP, the host detects the defined DHCP service and starts installing the operating system from its Kickstart tree.
...
This creates the host entry and the relevant provisioning settings. This also includes creating the necessary directories and files for UEFI booting the bare metal host. When you start the physical host and set its boot mode to UEFI, the host detects the defined DHCP service and starts installing the operating system from its Kickstart tree.
