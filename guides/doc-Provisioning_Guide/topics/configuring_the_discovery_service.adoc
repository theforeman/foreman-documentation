...
hosts or virtual machines can be created with incorrect network configurations. On Dell servers, new naming scheme can be disabled via `biosdevname=1` kernel command line option, for other hardware or virtual machines, new naming scheme can be completely turned off via `net.ifnames=1`.
