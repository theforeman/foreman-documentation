...
ifeval::["{build}" == "foreman"]
   ...
   * Virtual network running on the libvirt server. Only NAT and isolated virtual networks can be managed through {Project}.
endif::[]
...
To create a compute resource, enter the following command:
...
Create the compute profile with the `hammer compute-profile create` command if doesn't exist.
...
Then set the values for the compute profile with the `hammer compute-profile values create` command.
