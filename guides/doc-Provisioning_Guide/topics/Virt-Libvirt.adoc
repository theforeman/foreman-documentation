...
The requirements for KVM provisioning include:
  ...
  * Virtual network running on the libvirt server.
	endif::[]
  * An existing virtual machine image if you want to use image-based provisioning. Ensure that this image exists in a storage pool on the KVM host. The `default` storage pool is usually located in `/var/lib/libvirt/images`. Only directory pool storage types can be managed via {ProjectX}.
...
If you create a host using an image provisioning method, the new host entry triggers the KVM server to create the virtual machine, using the selected pre-existing image as a basis for the new volume. Alternatively, if a Finish template is associated to the host operating system, {Project} connects to the VM using SSH protocol and executes the script. This can only work if DHCP server is under {Project} control, or IP address of the provisioning interface was manually set to the IP address of the virtual host.
