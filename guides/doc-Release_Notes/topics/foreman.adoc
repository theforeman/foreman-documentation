...
TLS ciphers disregard user specification::
+
--
TLS 1.3 cipher settings allow unspecified ciphers, disregarding user specification, due to incompatibility with certain Ruby versions.
This affects builds running Ruby versions prior to 3.2 because these do not include the `SSL_CTX_set_ciphersuites()` method.
Debian 12 and Ubuntu 22.04 builds are impacted as they run Ruby versions lower than Ruby 3.2.
{EL} build of Ruby 3.0 is not affected.
Additionally, for all Ruby versions, when multiple TLS versions are allowed and the configured ciphers are only compatible with one version, the incompatible version is left completely unrestricted.
This potentially exposes data.
As a workaround, configure ciphers for both TLS 1.2 and 1.3.
--
