...
+
--
When users configure allowlists for TLS ciphers, {SmartProxy} does not enforce the policy for all TLS versions.
For {Project} on Debian 12 and Ubuntu 22.04, if you set ciphers for TLS 1.2, TLS 1.3 ciphers are unrestricted.
