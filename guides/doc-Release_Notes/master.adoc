include::common/attributes.adoc[]
include::common/header.adoc[]

ifndef::katello[]
include::topics/foreman.adoc[]
endif::[]

ifdef::katello[]
= Foreman {ProjectVersion} and Katello {KatelloVersion} Release Notes

include::topics/foreman.adoc[leveloffset=+1]
include::topics/katello.adoc[leveloffset=+1]
endif::[]

// Start inserting specific x.y.z releases here
include::topics/foreman-3.12.1.adoc[leveloffset=+1]
ifdef::katello[]
include::topics/katello-4.14.0.adoc[leveloffset=+1]
include::topics/katello-4.14.1.adoc[leveloffset=+1]
include::topics/katello-4.14.2.adoc[leveloffset=+1]
include::topics/katello-4.14.3.adoc[leveloffset=+1]
endif::[]
include::topics/foreman-3.12.0.adoc[leveloffset=+1]

[appendix]
[id="foreman-contributors"]
= Foreman Contributors

include::topics/foreman-contributors.adoc[leveloffset=+1]

ifdef::katello[]
[appendix]
[id="katello-contributors"]
= Katello Contributors

include::topics/katello-contributors.adoc[leveloffset=+1]
endif::[]
