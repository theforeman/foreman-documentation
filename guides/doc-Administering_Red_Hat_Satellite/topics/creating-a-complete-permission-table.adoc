[id='creating-a-complete-permission-table_{context}']
...
. Ensure that the required packages are installed. Execute the following command on the {ProjectServer}:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# {foreman-maintain} packages install tfm-rubygem-foreman*
----
...
. Press `*Ctrl*` + `*D*` to exit the {Project} console. Insert the following text at the first line of `/tmp/table.html`:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
