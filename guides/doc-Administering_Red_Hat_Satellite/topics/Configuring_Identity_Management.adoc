...
* Using {Keycloak} as an OpenID provider for external authentication to {Project} with CAC cards.
For more information, see xref:integrating-satellite-with-red-hat-single-sign-on-for-external-authentication[].
