...
For more information about support levels for Ansible in {Project}, see the _Support for Ansible in {Project}_ section of the {BaseURL}release_notes/chap-red_hat_satellite-release_notes-new_features_and_enhancements[New Features and Enhancements] in the _Release Notes_.
...
. Optional: To override the Ansible variable with {Project}, see the xref:sect-{Project_Link}-Administering_{Project_Link}-Managing_Ansible_Variables_with_{Project}[].
