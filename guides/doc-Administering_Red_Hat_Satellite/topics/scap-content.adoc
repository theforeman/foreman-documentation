...
You can either create SCAP content or obtain it from a vendor. Supported profiles are provided for Red{nbsp}Hat Enterprise{nbsp}Linux in the scap-security-guide package. The creation of SCAP content is outside the scope of this guide, but see the https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/7/html/Security_Guide/[Red Hat Enterprise Linux 7 Security Guide] for information on how to download, deploy, modify, and create your own content.

The default SCAP content provided with the OpenSCAP components of {ProjectX} depends on the version of Red{nbsp}Hat Enterprise{nbsp}Linux. On Red{nbsp}Hat Enterprise{nbsp}Linux{nbsp}7, content for both Red{nbsp}Hat Enterprise{nbsp}Linux{nbsp}6 and Red{nbsp}Hat Enterprise{nbsp}Linux{nbsp}7 is installed.
