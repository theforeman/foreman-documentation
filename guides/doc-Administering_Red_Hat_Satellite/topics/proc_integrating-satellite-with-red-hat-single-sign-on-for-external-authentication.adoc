...
. Enter the name of the external user group you created in the {Keycloak} web UI.
. Click *Submit*.
...
+
. Add the value for the Hammer client in the Open IDC audience. Note that if you have already registered another client with keycloak, make sure also to append that audience in the array. For example:
+
[options="nowrap", subs="+quotes,attributes"]
----
