...
You can configure {Project} to use {Keycloak} as an OpenID provider for external authentication with CAC cards.
You can only use CAC cards; other authentication methods are not supported.
...
Add a *Valid redirect URI* in the following form: `https://{foreman-example-com}/users/extlogin`.
.. Click *Save*.

.. Click the *Mappers* tab and click *Create*.
Set the following values for the audience mapper:
+
...
.. Click the *Mapper* tab and click *Create* to add a group mapper so that you can specify authorization in {Project} based on group membership.
Set the following values for the group mapper:
+
...
+
* Enter the *Username*, *Email*, and other user details.
Set the *User Enabled* toggle to *ON*.
+
* Click *Save*.
...
Select the group you want to join from the *Available Groups* list, and click *Join*.
The user is now part of the selected group.
