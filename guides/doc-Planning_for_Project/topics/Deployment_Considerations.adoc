...
The first step to a working {Project} infrastructure is installing an instance of {ProjectServer} on a dedicated {RHEL} 9 server.
...
The most common deployment scenarios are listed in xref:[common-deployment-scenarios_planning].
...
In such case, {SmartProxies} have to integrate with those external services.

ifdef::satellite[]
* *Is my {ProjectServer} required to be disconnected from the Internet?* – Disconnected {Project} is a common deployment scenario.
