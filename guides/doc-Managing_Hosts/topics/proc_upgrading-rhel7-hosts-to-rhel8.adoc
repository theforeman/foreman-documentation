...
. When the check is finished, click the *Leapp preupgrade report* tab to see if LEAPP has found any issues on RHEL 7 hosts. Issues might break the upgrade procedure. Issues that have the *Inhibitor* flag are concidered crucial. Some issues might have documentation linked that describe how to fix them.
