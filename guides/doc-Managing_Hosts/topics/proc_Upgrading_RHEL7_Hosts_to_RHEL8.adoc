...
* Enable remote execution feature on {Project}. For more information, see xref:chap-Managing_Hosts-Running_Remote_Jobs_on_Hosts[].
...
. When the check is finished, click the *Leapp preupgrade report* tab and fix all issues. Note that you can fix some issues that has remediation automatically. You need to fix other issues manually. When the upgrade check is finished and your RHEL 7 hosts do not have any issues, you can proceed with the upgrade.
