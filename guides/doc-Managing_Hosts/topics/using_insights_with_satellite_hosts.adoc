...
By default, {Project} automatically registers all hosts to Insights.
This happens when you register hosts to {Project}.
If you do not want your hosts to be registered to Insights, you can opt-out from it during hosts registration.

You can also register hosts to Insights manually using Puppet or an Ansible role.
...
To register your hosts to Insights using an Ansible role, proceed to the procedure below.

.Deploying Red{nbsp}Hat Insights using the Ansible Role

If you opt-out from automatic hosts registration to Insights that heppens when hosts are registered to {Project}, you can register hosts with Red{nbsp}Hat Insights using the *RedHatInsights.insights-client* Ansible role.
