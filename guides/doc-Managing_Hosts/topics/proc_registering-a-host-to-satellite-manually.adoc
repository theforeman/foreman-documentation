[id="Registering_a_Host_Manually_{context}"]
...
{RHEL} hosts register to the Red{nbsp}Hat Content Delivery Network by default.
...
. Clear any old host data related to Subscription Manager:
+
...
[NOTE]
====
For {RHEL} 6.3 hosts, the release version defaults to {RHEL}{nbsp}6 Server and needs to be pointed to the 6.3 repository:
