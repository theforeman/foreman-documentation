...
* *Mode*: Select the bonding mode that defines a policy for fault tolerance and load balancing. See xref:Bonding_Modes_Available_in_Red_Hat_Satellite[] for a brief description of each bonding mode.
...
.Bonding Modes Available in Red Hat Satellite
[options="header"]
|====
