...
--request GET --user _My_User_Name_:_My_Password_ --insecure \
...
[options="nowrap", subs="+quotes,attributes"]
----
$ curl --request GET --user _My_User_Name_:_My_Password_ --insecure \
...
$ curl --header "Accept:application/json" \
--header "Content-Type:application/json" \
--request POST --user _My_User_Name_:_My_Password_ --insecure \
...
----
+
In the command output, you can see the ID for this lifecycle environment is `2`, and the lifecycle environment prior to this one is `1`.
...
----
+
In the command output, you can see the ID for this lifecycle environment is `3`, and the lifecycle environment prior to this one is `2`.
...
[options="nowrap", subs="+quotes,attributes"]
----
$ curl --request DELETE --user _My_User_Name_:_My_Password_ --insecure \
