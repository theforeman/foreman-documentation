...
<6> When downloading content from Satellite Server, specify the output file with the `--output` option.
<7> Use the API route in the following format: `https://_satellite.example.com_/katello/api/activation_keys`. In Satellite 6, version 2 of the API is the default. Therefore it is not necessary to use `v2` in the URL for API calls.
...
https://_satellite.example.com_/api/hosts | python3 -m json.tool
----
...
https://_satellite.example.com_/katello/api/activation_keys \
| python3 -m json.tool
----
+
Example response:
...
--user _sat_username:sat_password_ --insecure \
--data @activation-key.json \
https://_satellite.example.com_/katello/api/activation_keys/2 \
| python3 -m json.tool
----
+
Example response:
