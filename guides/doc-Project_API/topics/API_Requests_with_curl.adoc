...
Red Hat Satellite requires the use of HTTPS, and by default a certificate for host identification. If you have not added the Satellite Server certificate as described in xref:Authenticating_API_Calls-SSL_Authentication_Overview[], then you can use the `--insecure` option to bypass certificate checks.

For user authentication, you can use the `--user` option to provide Satellite user credentials in the form `--user _username:password_` or, if you do not include the password, the command prompts you to enter it. To reduce security risks, do not include the password as part of the command, because it then becomes part of your shell history. Examples in this section include the password only for the sake of simplicity.
...
This section outlines how to use `curl` with the Satellite{nbsp}6 API to request information from your Satellite deployment. These examples include both requests and responses. Expect different results for each deployment.
...
https://_satellite.example.com_/api/users | python3 -m json.tool
----
...
This section outlines how to use `curl` with the Satellite{nbsp}6 API to manipulate resources on the Satellite{nbsp}Server. These API calls require that you pass data in `json` format with the API call. For more information, see xref:sect-API_Guide-Passing_JSON_Data_with_the_API_Request[].
...
https://_satellite.example.com_/api/users/_8_ | python3 -m json.tool
----
