...
. Examine the following output. Each Smart Class Parameter has an ID that is global for the same {Project} instance. The `content` parameter of the `motd` class has `id=3` in this {ProjectServer}. Do not confuse this with the Puppet Class ID that displays before the Puppet Class name.
+
Example response:
+
