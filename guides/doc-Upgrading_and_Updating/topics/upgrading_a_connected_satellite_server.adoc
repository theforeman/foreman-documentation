...
----
ifdef::katello[]
. Optional: If you use PostgreSQL as an external database, on the PostgreSQL server, install the `postgresql-evr` package:
+
...
----
ifdef::katello[]
# dnf update https://yum.theforeman.org/releases/{ProjectVersion}/el8/x86_64/foreman-release.rpm \
