...
For {SmartProxies}, you can choose to re-deploy new 6.10 {SmartProxies} rather than to upgrade.
...
[NOTE]
====
If you are planning to avoid the upgrade from Pulp 2 to Pulp 3 and deploy a new {Project} 6.10 infrastructure due to the Pulp 3 changes instead, you may want to wait for {Project} 7.0 to deploy with RHEL 8 directly.
====
endif::[]
