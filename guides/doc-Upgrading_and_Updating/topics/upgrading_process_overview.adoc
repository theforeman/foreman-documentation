...
You can upgrade to {ProjectName} from the previous version. {ProjectServer}s and {SmartProxyServer}s on earlier versions must first be upgraded to the {Project} version previous to this. For more information, see the {Project} {ProductVersionPrevious} {UpgradingDocURL}upgrading_and_updating_red_hat_satellite/[Upgrading and Updating {ProjectName}] guide.
