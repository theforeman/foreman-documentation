...
ifdef::satellite[]
NOTE: {Project} {ProjectVersion} is released only on {EL} 8.
If you are running {Project} {ProjectVersionPrevious} on {EL} 7, you must upgrade to {EL} 8 before you can upgrade to {Project} {ProjectVersion}.
For more information about upgrading from {EL} 7 to {EL} 8, see https://access.redhat.com/documentation/en-us/red_hat_satellite/{ProjectVersionPrevious}/html/upgrading_and_updating_red_hat_satellite/upgrading-satellite-or-proxy-in-place-using-leapp_upgrade-guide[Upgrading Satellite or Capsule {ProjectVersionPrevious} to {RHEL} 8 In-Place Using Leapp].
endif::[]
