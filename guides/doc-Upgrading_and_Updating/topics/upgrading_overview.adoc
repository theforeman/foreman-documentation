...
You must upgrade {Project} {ProjectVersionPrevious} from {EL} 7 to {EL} 8.
For more information about upgrading from {EL} 7 to {EL} 8, see https://access.redhat.com/documentation/en-us/red_hat_satellite/6.11/html/upgrading_and_updating_red_hat_satellite/upgrading-satellite-or-proxy-in-place-using-leapp_upgrade-guide[Upgrading Satellite or Capsule to {RHEL} 8 In-Place Using Leapp].
endif::[]
