...
During migration to Pulp3, amount of data stored in the `/var/lib/pulp` directory can be doubled.
Ensure there is enough space on the `/var/lib/pulp` volume.
After Pulp2 is removed, the extra space can be claimed back, however, shrinking of XFS volumes is not possible and copying data to a different partition might me necessary in order to bring volume size down to the previous value.
