...
There are several causes of corrupted or missing content:
  ...
  * There is some mismatch between subsystems in {Project} such as Katello and Pulp.
  This can happen if a repository or content view is improperly deleted by skipping steps in a paused Foreman Task.
...
+
If there are a large percentage of missing or corrupted RPMs, for example, more than 5-10%, {Team} recommends that you review the list generated and contact support for guidance.
+
...
If this is the first time you have run `{foreman-maintain} content prepare`, {Team} recommends running it a second time to try to reduce the number of corrupt or missing RPMs.
...
You can determine how near the process is to completion by using the following command:
+
[options="nowrap", subs="verbatim,quotes,attributes"]
----
# {foreman-maintain} content migration-stats
