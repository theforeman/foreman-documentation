...
During the upgrade process, {Project} attempts to locate macros that are deprecated for {Project} {ProductVersion} and converts old syntax to new syntax for the default {Product} templates, parameters, and lookup keys and values. However, {Product} does not convert old syntax in the custom templates that you have created and in the cloned templates.
