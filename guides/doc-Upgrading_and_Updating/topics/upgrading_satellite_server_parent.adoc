...
ifdef::satellite[]
However, to access the latest migration and tooling fixes, {Team} recommends that you upgrade to the latest minor version of {ProjectServer} before upgrading from {ProjectVersionPrevious} to {ProjectVersion}.
...
You can reduce this time by following the procedure in xref:migrating_pulp_content[]
...
ifdef::satellite[]
include::migrating_pulp_content.adoc[leveloffset=+1]
endif::[]