ifdef::context[:parent-context: {context}]

include::modules/con_preparing-your-environment-for-smart-proxy-installation.adoc[]

include::modules/ref_operating-system-requirements.adoc[leveloffset=+1]

include::modules/ref_system-requirements.adoc[leveloffset=+1]

ifdef::katello,satellite[]
include::modules/ref_smart-proxy-storage-requirements.adoc[leveloffset=+1]

include::modules/ref_best-practices-for-optimizing-storage.adoc[leveloffset=+1]
endif::[]

ifdef::containerized[]
include::common/modules/proc_serving-smart-proxy-container-images-from-project-server.adoc[leveloffset=+1]
endif::[]

include::modules/proc_opening-required-ports.adoc[leveloffset=+1]

ifdef::parent-context[:context: {parent-context}]
ifndef::parent-context[:!context:]
