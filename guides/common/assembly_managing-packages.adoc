:_mod-docs-content-type: ASSEMBLY

include::modules/con_managing-packages-in-project.adoc[]

include::modules/proc_enabling-and-disabling-repositories-on-hosts.adoc[leveloffset=+1]

include::modules/proc_viewing-upgradable-packages-by-using-web-ui.adoc[leveloffset=+1]

include::modules/proc_viewing-upgradable-packages-by-using-cli.adoc[leveloffset=+1]

include::modules/proc_installing-packages-on-a-host.adoc[leveloffset=+1]

include::modules/proc_upgrading-packages-on-a-host.adoc[leveloffset=+1]

include::modules/proc_removing-packages-from-a-host.adoc[leveloffset=+1]

include::modules/proc_managing-packages-on-a-host-by-using-api.adoc[leveloffset=+1]
