...
. xref:installing-the-azurerm-plugin_{context}[].
endif::[]
. xref:adding-azure-connection_{context}[].
. xref:adding-images-to-server_{context}[].
. xref:adding-azure-details-to-a-compute-profile_{context}[].
. xref:creating-image-only-hosts_{context}[].
