[id="installing-satellite-server-disconnected_{context}"]
= Installing {ProjectServer}

When the intended host for the {ProjectServer} is in a disconnected environment, you can install the {ProjectServer} by using an external computer to download an ISO image of the packages, and copying the packages to the system you want to install {ProjectServer} on. This method is not recommended for any other situation as ISO images might not contain the latest updates, bug fixes, and functionality.
...
include::modules/performing_initial_configuration_sat_server_manual_disconnected.adoc[leveloffset=+2]
