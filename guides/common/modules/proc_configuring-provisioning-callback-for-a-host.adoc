...
[id="configuring-provisioning-callback-for-a-host"]
...
.Prerequisites
* {Project} is added as a dynamic inventory in {awx}.
ifndef::orcharhino[]
For more information, see xref:integrating-{project-context}-and-{awx-id}[].
endif::[]
