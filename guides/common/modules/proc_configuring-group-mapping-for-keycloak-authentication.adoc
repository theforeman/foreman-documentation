...
[role="_abstract"]
Optionally, to implement the role-based access control (RBAC), create a group in {Project}, assign a role to this group, and map an {keycloak} group to the {Project} group.
Anyone in the given group in {keycloak} will log in under the corresponding {Project} group.
