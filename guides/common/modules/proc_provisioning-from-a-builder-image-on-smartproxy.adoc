...
For more information, see {InstallingSmartProxyDocURL}adding-lifecycle-environments-by-using-web-ui[Adding lifecycle environments to {SmartProxyServers}] in _{InstallingSmartProxyDocTitle}_.

.Procedure
. On {Project}, create a {customproduct}, add a {customfiletype} repository to this product, and upload the image to the repository.
For more information, see {ContentManagementDocURL}importing-an-iso-image-by-using-web-ui[Importing an ISO image by using {ProjectWebUI}] in _{ContentManagementDocTitle}_.
