...
To send email messages from {ProjectServer}, you can use an SMTP server or the deprecated `sendmail` mail transfer agent (MTA).

.Prerequisites
* Some SMTP servers with anti-spam protection or gray-listing features are known to cause problems.
To set up outgoing email with such a service, install and configure an uncustomized SMTP service on {ProjectServer} for relay. Alternatively, you can use the deprecated `sendmail` command
