...
[NOTE]
====
While you can configure the LDAP server integrated with {FreeIPA} as an external authentication source, {IPA} users will not be able to log in using single sign-on.
Instead, consider configuring {IPA} as an external identity provider.
See xref:configuring-a-freeipa-server-as-an-external-identity-provider-for-project_{context}[].
====

[IMPORTANT]
====
Users cannot use both {IPA} and LDAP as an authentication method.
