:_mod-docs-content-type: PROCEDURE

[id="running-an-ansible-playbook-from-{project-context}_{context}"]
= Running an Ansible Playbook from {Project}

[role="_abstract"]
You can run an Ansible Playbook on a host or host group by executing a remote job in {Project}.

[IMPORTANT]
====
When you execute an Ansible Playbook on multiple hosts, {Project} renders the playbook for all hosts in the batch, but only uses the rendered playbook of the first host to execute it on all hosts in the batch.
Therefore, you cannot modify the behavior of the playbook per host by using a host parameter in the template control flow constructs.
Host parameters are translated into Ansible variables, so you can use them to control the behavior in native Ansible constructs.
ifndef::orcharhino[]
For more information, see link:https://bugzilla.redhat.com/show_bug.cgi?id=2282275[BZ#2282275].
endif::[]
====

.Prerequisites
* Ansible plugin in {Project} is enabled.
* Remote job execution is configured.
ifdef::managing-hosts[]
For more information, see xref:configuring-and-setting-up-remote-jobs[].
endif::[]
ifndef::managing-hosts[]
For more information, see {ManagingHostsDocURL}configuring-and-setting-up-remote-jobs[Configuring and Setting Up Remote Jobs] in _{ManagingHostsDocTitle}_.
endif::[]
* You have an Ansible Playbook ready to use.

.Procedure
. In the {ProjectWebUI}, navigate to *Monitor* > *Jobs*.
. Click *Run Job*.
. In *Job category*, select `Ansible Playbook`.
. In *Job template*, select `Ansible - Run playbook`.
. Click *Next*.
. Select the hosts on which you want to run the playbook.
. In the *playbook* field, paste the content of your Ansible Playbook.
. Follow the wizard to complete setting the remote job.
ifdef::managing-hosts[]
For more information, see xref:configuring-and-setting-up-remote-jobs[].
endif::[]
ifndef::managing-hosts[]
For more information, see {ManagingHostsDocURL}configuring-and-setting-up-remote-jobs[Configuring and Setting Up Remote Jobs] in _{ManagingHostsDocTitle}_.
endif::[]
. Click *Submit* to run the Ansible Playbook on your hosts.

.Example Ansible Playbook to access host parameters
====
You can use host parameters from {Project} in Ansible Playbooks.

[source, yaml, options="nowrap", subs="+quotes,verbatim,attributes"]
----
---
- name: "Ansible Playbook to access host parameters"
  hosts: all
  tasks:
    - name: "View a host parameter"
      ansible.builtin.debug:
        msg: "{{ hostvars[inventory_hostname]['_My_Host_Parameter_'] }}"
...
----

Ansible will access the host parameter and show its value.
If you set the host parameter to an ERB template, for example, `<%= @host.capabilities.include?(:snapshots) %>`, {Project} will render it and provide the result to Ansible.
Ansible receives only the rendered value, not the ERB template itself.

ifdef::managing-hosts[]
For more information, see xref:Host_Parameter_Hierarchy_{context}[].
endif::[]
ifndef::managing-hosts[]
For more information about host parameters, see {ManagingHostsDocURL}Host_Parameter_Hierarchy_{context}[Host Parameter Hierarchy] in _{ManagingHostsDocTitle}_.
endif::[]
====

.Additional resources
ifdef::managing-hosts[]
* xref:importing-an-ansible-playbook-by-name_{context}[]
* xref:importing-all-available-ansible-playbooks_{context}[]
endif::[]
ifndef::managing-hosts[]
For information about importing Ansible Playbooks as job templates for remote execution, see {ManagingHostsDocURL}configuring-and-setting-up-remote-jobs[Configuring and Setting Up Remote Jobs] in _{ManagingHostsDocTitle}_.
endif::[]
