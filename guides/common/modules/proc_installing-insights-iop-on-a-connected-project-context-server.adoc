...
----
ifdef::satellite[]
. To use the {insights-iop} vulnerability service, and if your {ProjectServer} connects to `\https://security.access.redhat.com` through an HTTP proxy, configure the `iop-cvemap-download` service for the same HTTP proxy.
