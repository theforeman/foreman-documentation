...
----
ifdef::satellite[]
. To use the {insights-iop} vulnerability service, and if your {ProjectServer} connects to `\https://security.access.redhat.com` through an HTTP proxy, configure the `iop-cvemap-download` service for the same HTTP proxy. To do so, enter the `systemctl edit iop-cvemap-download.service` command and input the following content:
+
[source, ini, options="nowrap", subs="+quotes,verbatim,attributes"]
----
[Service]
Environment = HTTPS_PROXY=_http://proxy.example.com:3128_
Environment = NO_PROXY=localhost
----
+
:FeatureName: The {insights-iop} vulnerability service
