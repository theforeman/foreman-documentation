...
[id="configuring-{smart-proxy-context}-to-provision-{client-os-context}-by-using-uefi"]
= Configuring {SmartProxy} to provision {client-os} by using UEFI

[role="_abstract"]
When provisioning, always use the boot loader version that is provided by the operating system that is being provisioned, not the one which {ProjectName} was deployed on.

This is especially important when using Secure Boot, which follows a chain of trust from the start of the host to the loading of Linux kernel modules.
...
ifdef::satellite[]
You can provision the same RHEL version as your {ProjectServer} on hosts out of the box and you can skip this procedure.
...
[IMPORTANT]
====
You have to perform the following configuration steps on each TFTP {SmartProxy} for a subnet to provision hosts using UEFI on that subnet.
====
...
.Next steps
* You can now provision {client-os} hosts by using the *Grub2 UEFI* and *Grub2 UEFI HTTPS* PXE loaders.
ifdef::satellite[]
