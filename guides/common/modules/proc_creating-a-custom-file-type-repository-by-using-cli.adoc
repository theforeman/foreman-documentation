...
[role="_abstract"]
You can add a {customfiletype} repository to an existing {customproduct}.
Select the `file` repository type and follow the same workflow as when you add other {customcontent}.
...
| `--mirror-on-sync` _boolean_         | Must this repo be mirrored from the source, and stale packages removed, when synced?
Set to `true` or `false`, `yes` or `no`, `1` or `0`.
