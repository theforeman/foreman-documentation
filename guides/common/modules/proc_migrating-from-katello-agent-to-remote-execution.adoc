...
.Procedure
. _If you have Remote Execution configured to use `pull-mqtt` mode_: Deploy the remote execution pull client to the hosts.
Follow the procedure in xref:setting-up-pull-client_{context}[].
