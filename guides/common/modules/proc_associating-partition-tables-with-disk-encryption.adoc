...
[id="associating-partition-tables-with-disk-encryption"]
= Associating partition tables with disk encryption

[role="_abstract"]
You can encrypt host disks by using Linux Unified Key Setup (LUKS) during provisioning to protect data at rest.
Optionally, bind the disk to a Tang server through Clevis for decryption during boot.
