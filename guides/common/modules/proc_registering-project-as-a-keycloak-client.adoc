...
= Registering {Project} as a {Keycloak-SSO} client
Use this procedure to register {Project} to {Keycloak-SSO} as a client and configure {Project} to use {Keycloak-SSO} as an authentication source.

You can configure {Project} and {Keycloak-SSO} with two different authentication methods:

. Users authenticate to {Project} using the {ProjectWebUI}.
. Users authenticate to {Project} using the {Project} CLI.

You must decide on how you want your users to authenticate in advance because both methods require different {Project} clients to be registered to {Keycloak-SSO} and configured.
The steps to register and configure {Project} client in {Keycloak-SSO} are distinguished within the procedure.

You can also register two different {Project} clients to {Keycloak-SSO} if you want to use both authentication methods and configure both clients accordingly.
...
. Register {Project} to {Keycloak-SSO} as a client.
Note that you the registration process for logging in using the web UI and the CLI are different.
You can register two clients {Project} clients to {Keycloak-SSO} to be able to log in to {Project} from the web UI and the CLI.
+
...
This command creates a client for {Project} in {Keycloak-SSO}.
+
Then, configure {Project} to use {Keycloak-SSO} as an authentication source:
+
[options="nowrap", subs="verbatim,quotes,attributes"]
----
