...
No reports::
When there are no reports but the host uses a configuration management system, for example, proxy is associated or `always_show_configuration_status` setting is set to true, it maps to *Warning*.
