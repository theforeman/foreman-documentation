...
[id="renewing-a-custom-ssl-certificate-on-{smart-proxy-context}-server"]
= Renewing a custom SSL certificate on {SmartProxyServer}

[role="_abstract"]
You can deploy a renewed custom SSL certificate on each {SmartProxyServer} before it expires to maintain secure communication with {ProjectServer}.
Run a unique `{foreman-installer}` command on each server because the output of `{certs-generate}` cannot be reused.

.Prerequisites
* You have created a new Certificate Signing Request and sent it to the Certificate Authority to sign the certificate.
