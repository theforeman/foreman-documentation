...
[role="_abstract"]
ifdef::containerized[]
You can create an offline backup of your {ProjectServer} and {SmartProxyServer}.
endif::[]
ifndef::containerized[]
Choose a backup method based on how long you can keep {Project} services unavailable and how much time you can allocate for the backup.
endif::[]
...
You can skip this action by using the `--skip-pulp-content` option with the `{foremanctl} backup` command.
* Service restart to restore all {Project} services.
endif::[]
ifndef::containerized[]
+
...
The require less time and storage than a full offline backup.
ifdef::containerized[]
+
