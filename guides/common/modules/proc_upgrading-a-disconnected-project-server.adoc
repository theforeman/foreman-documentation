...
... Download and mount the {Project} ISO image and set up the local repositories.
For more information, see {InstallingServerDisconnectedDocURL}installing-from-the-offline-repositories_{project_context}[Installing the {Project} packages from the offline repositories] in _{InstallingServerDisconnectedDocTitle}_.
