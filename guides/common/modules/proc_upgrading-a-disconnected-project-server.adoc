...
----
. Run the health check option to determine if the system is ready for the update.
The first time you run this command, `{foreman-maintain}` prompts you for the hammer admin user credentials and saves them in `/etc/foreman-maintain/foreman-maintain-hammer.yml`.
+
[options="nowrap" subs="+quotes,verbatim,attributes"]
----
...
This allows you to monitor progress without keeping a continuous connection to the command shell.
If you lose connection to the command shell running the update command, check the logged messages in `{installer-log-file}` to confirm whether the process completed successfully.
