...
[id="webhook-behavior-and-integration"]
= Webhook behavior and integration

[role="_abstract"]
You can use webhooks as a way to integrate {Project} with external systems.
For example, webhooks can integrate with monitoring systems to send alerts when a host is created or deleted.
...
* `@payload`: Payload data, different for each event type.
To access individual fields, use `@payload[:key_name]` Ruby hash syntax.
* `@payload[:object]`: Database object for events triggered by database actions (create, update, delete).
Not available for custom events.
...
* xref:webhooks-available-events_{context}[]
