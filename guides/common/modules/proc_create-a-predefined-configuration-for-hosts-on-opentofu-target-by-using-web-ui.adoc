...
.Prerequisites
* You created an image for {compute-resource-target-name} on {Project}.
For more information, see xref:define-an-image-for-hosts-on-{compute-resource-target-id}-by-using-web-ui[].
