[id="{project-context}-server-as-red-hat-insights-client_{context}"]
= {ProjectServer} as Red{nbsp}Hat Insights client
...
. The Insights client is pre-installed on {EL} 9.
+
...
+
. To unregister {ProjectServer} from Red{nbsp}Hat Insights, enter the following command:
+
[options="nowrap" subs="+quotes,attributes"]
----
