...
. Power off the source server.
. Register the system to the Customer Portal, attach subscriptions, and enable only the required subscriptions:
** If the target server runs {EL}{nbsp}9, enter the following commands:
+
[options="nowrap" subs="quotes,attributes"]
----
...
----
** If the target server runs {EL}{nbsp}8, enter the following commands:
+
[options="nowrap" subs="quotes,attributes"]
----
# subscription-manager register _your_customer_portal_credentials_
