...
[id="exporting-a-repository-incrementally-in-the-syncable-format"]
= Exporting a repository incrementally in the syncable format

[role="_abstract"]
You can export only changed repository content in syncable format to reduce transfer size when serving content from a custom CDN over HTTP/HTTPS.
