...
[id="install-and-configure-{FreeIPA-context}-packages-on-{project-context}-server-or-{smart-proxy-context}-server"]
= Install and configure {FreeIPA} packages on {ProjectServer} or {SmartProxyServer}
