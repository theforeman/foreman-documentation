...
[id="provisioning-virtual-machines-on-kvm"]
= Provisioning virtual machines on KVM

[role="_abstract"]
You can provision hosts and control virtualization functions by using the `libvirt` API on the KVM hypervisor.

You can also manage virtual machines created outside of {Project} by importing them or associating them with existing hosts.
...
Before you provision virtual machines on KVM, ensure that your environment meets the following requirements:
...
* xref:preparing-networking-for-host-management[]
* {ManagingHostsDocURL}Associating_a_Virtual_Machine_from_a_Hypervisor_managing-hosts[Associating a virtual machine with {Project} from a hypervisor] in _{ManagingHostsDocTitle}_
ifdef::satellite[]
* {RHELDocsBaseURL}8/html/configuring_and_managing_virtualization/index[_{RHEL}{nbsp}8 Configuring and managing virtualization_]
endif::[]
