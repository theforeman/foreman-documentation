...
= Configuring {Project} settings for {Keycloak-SSO} authentication using the CLI

Use this procedure to configure {Project} settings for {Keycloak-SSO} authentication using the {Project} CLI.
...
* Ensure that the *Access Type* setting in the {Project} client in the {Keycloak-SSO} web UI is set to *public*.
...
. Set the algorithm for encoding on {Keycloak-SSO}, for example, `RS256`:
+
----
...
[NOTE]
====
If you register several {Keycloak-SSO} clients to {Project}, ensure that you append all audiences in the array.
For example:
[options="nowrap", subs="+quotes,attributes"]
----
# hammer settings set --name oidc_audience \
...
. Retrieve the ID of the {Keycloak-SSO} authentication source:
+
----
