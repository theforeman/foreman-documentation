...
.Procedure
. On the {SmartProxyServer}, locate the directory that {SmartProxy} uses for remote execution by cheking where the configured {SmartProxy} SSH private key is stored.
...
. On the {SmartProxyServer}, generate a new SSH key pair in the directory found in the previous step:
+
...
[NOTE]
====
If you created the new SSH key pair with the same name as the old one and thus replaced the old key pair, you do not need to update the {SmartProxy} configuration with {foreman-installer}.
====
