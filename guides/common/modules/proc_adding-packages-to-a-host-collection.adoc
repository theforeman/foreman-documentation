...
[role="_abstract"]
You can add packages to a host collection so that the packages are installed on all hosts in the collection in one action.

.Prerequisites
* The content to be added should be available in one of the existing repositories or added before this procedure.
