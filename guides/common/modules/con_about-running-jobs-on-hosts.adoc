...
Enable and configure the `pull-mqtt` pull mode on the {SmartProxy}.
When using `pull-mqtt` for pull mode, the remote client initiates all connections, and there are no outgoing connections from the proxy.
{Project} delegates pull jobs to the proxy, and the proxy notifies the host of a new job over the Message Queueing Telemetry Transport (MQQT).
The target host then pulls and runs the job.
Note that this pull mode is only applicable to Script provider jobs.
