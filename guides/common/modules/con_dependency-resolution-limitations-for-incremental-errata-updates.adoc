...
[id="dependency-resolution-limitations-for-incremental-errata-updates"]
...
After you add errata to a content view by using an incremental update, as described in the procedures above, be aware of the following dependency resolution limitation.
This limitation is separate from publish-time dependency solving for content views.
...
Because you can install all these packages by using `{client-package-manager}`, no potential problem is detected.
endif::[]
ifdef::katello[]
Because you can install all these packages by using the package manager of your host, no potential problem is detected.
endif::[]
