...
[id="customizing-the-discovery-pxe-boot"]
= Customizing the Discovery PXE boot

[role="_abstract"]
You can add specific configuration information as kernel parameters into the boot process of hosts during automatic hardware detection.
This is useful, for example, to direct the host to connect to specific {SmartProxies} or to provide additional network configuration.
...
For information about the kernel parameters, see xref:kernel-parameters-for-discovery-customization[].
