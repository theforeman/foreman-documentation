...
[role="_abstract"]
You synchronize Oracle Linux packages from Unbreakable Linux Network (ULN) channels using uln:// URLs and mandatory upstream credentials in {Project}.
Use the channel labels from the ULN web UI as the path after the protocol prefix when you add each repository.
