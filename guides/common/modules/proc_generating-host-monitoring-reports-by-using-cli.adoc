...
[role="_abstract"]
You can generate host monitoring reports by using Hammer CLI so that you can view host data, such as installed packages or compliance status, in a report template.
...
If your generated report is larger than the *Report auto gzip threshold (MiB)* setting, the `hammer report-template schedule` command downloads your report as a compressed `.gz` file.
