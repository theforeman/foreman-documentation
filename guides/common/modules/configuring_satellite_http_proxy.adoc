...
If your network uses an HTTP Proxy, you can configure Satellite Server to use an HTTP proxy for requests to the Red{nbsp}Hat Content Delivery Network (CDN) or another content source. Use the FQDN instead of the IP address where possible to avoid losing connectivity because of network changes.
...
. Verify that Satellite Server can connect to the Red{nbsp}Hat CDN and can synchronize its repositories.
...
|====
+
Satellite Server communicates with the Red{nbsp}Hat CDN securely over SSL. Use of an SSL interception proxy interferes with this communication. These hosts must be whitelisted on the proxy.
+
...
SELinux ensures access of Red{nbsp}Hat Satellite 6 and Red{nbsp}Hat Subscription Manager only to specific ports. In the case of the HTTP cache, the TCP ports are 8080, 8118, 8123, and 10001 - 10010. If you use a port that does not have SELinux type `http_cache_port_t`, complete the following steps:
