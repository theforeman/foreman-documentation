...
[id="prerequisites-for-using-ipxe"]
= Prerequisites for using iPXE

[role="_abstract"]
Before you use iPXE to boot virtual machines, verify that your environment meets the following requirements.
...
For booting virtual machines in UEFI mode by using HTTP, you can follow xref:creating-hosts-with-uefi-http-boot-provisioning-by-using-web-ui[] instead.
