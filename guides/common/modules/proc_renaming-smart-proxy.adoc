...
Use the `{project-change-hostname}` script and update certificates and UI references to the new hostname.
...
ifndef::orcharhino[]
For more information, see {InstallingSmartProxyDocURL}configuring-{smart-proxy-context}-server-with-a-custom-ssl-certificate[Configuring {SmartProxyServer} with a custom SSL certificate] in _{InstallingSmartProxyDocTitle}_.
endif::[]
