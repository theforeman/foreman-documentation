...
Both commands must return the FQDN of {SmartProxyServer} or the `{project-change-hostname}` script will fail to complete.
+
...
.. On your DNS server, add a record for {SmartProxyServer}'s new host name, and delete the record of the previous host name.
