...
[role="_abstract"]
Rename {SmartProxyServer} when its hostname or domain changes so that {Project} components and registered hosts continue to communicate correctly.
Use the `{project-change-hostname}` script and update certificates and references to the new hostname.
...
.Prerequisites
* You have a backup of your {SmartProxyServer} before renaming.
If you fail to successfully rename it, restore it from the backup.
For more information, see xref:backing-up-{project-context}[].
* The `hostname` and `hostname -f` commands on {SmartProxyServer} both return the FQDN of {SmartProxyServer}.
...
ifndef::orcharhino[]
For more information, see {InstallingSmartProxyDocURL}configuring-{smart-proxy-context}-server-with-a-custom-ssl-certificate_connected[Configuring {SmartProxyServer} with a custom SSL certificate] in _{InstallingSmartProxyDocTitle}_.
endif::[]
