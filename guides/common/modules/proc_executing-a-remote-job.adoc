...
You can also set up recurring jobs from the new Host page.
...
. On the *Ansible* page, select *Jobs*.
. Click the *Schedule recurring jobs* button.
. In the *Create New Recurring Ansible Run* window, you can define start time and date, and the frequency of runs.
. Click *Submit*.

The job will now be visible on the Ansible jobs page.
