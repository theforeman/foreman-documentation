...
ifdef::orcharhino[]
. On the *Advanced* tab, in the *Repositories* field, enter the {project-client-name} repository.
+
ifdef::debian,ubuntu[]
** In the *Repository* field, enter the {project-client-name} repository, for example, `deb \http://{foreman-example-com}/pulp/content/_My_Organization_Label_/Library/custom/{client-os-label}_client/{client-os-label}_client/ stable main`.
** In the *Repository GPG key URL* field, enter the URL of the `pulp_deb_signing.key` file on your {SmartProxy} as GPG public key to verify synchronized {client-content-type} content, for example, `\https://{foreman-example-com}/pub/pulp_deb_signing.key`.
endif::[]
