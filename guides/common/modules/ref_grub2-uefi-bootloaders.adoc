...
= GRUB2 UEFI boot loaders and the `bootloader-universe` structure

This section describes GRUB-based PXE boot loaders, including *PXEGrub (Legacy)* and *PXEGrub2*,  
and explains how the `bootloader-universe` directory structure organizes GRUB2 binaries by OS and architecture.

== Overview

{Project} supports two generations of GRUB-based PXE boot loaders: *PXEGrub (GRUB Legacy)* and *PXEGrub2 (GRUB2-based)*.
PXEGrub is used for older BIOS or early UEFI systems, while PXEGrub2 supports *UEFI* and *Secure Boot* environments.

Both are managed by the {SmartProxy}’s TFTP service, which provides PXE clients with the appropriate boot loader binaries and configuration files.
