...
[id="adding-a-microsoft-azure-connection-to-{project-context}-server-by-using-cli"]
= Adding a Microsoft Azure connection to {ProjectServer} by using Hammer CLI

[role="_abstract"]
Add Microsoft Azure as a compute resource to provision and manage hosts on Azure.
Note that you must add a separate compute resource for each Microsoft Azure region that you want to use.

.Procedure
* Add an Azure compute resource to {Project}:
+
