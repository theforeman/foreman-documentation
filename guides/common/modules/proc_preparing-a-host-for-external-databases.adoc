...
* The {RHEL} 7 server must meet {Project}'s link:{InstallingProjectDocURL}preparing-environment-for-satellite-installation#satellite-storage-requirements_satellite[Storage Requirements].

.Procedure

. Use the instructions in link:{InstallingProjectDocURL}installing-satellite-server-connected#attaching-satellite-infrastructure-subscription_satellite[Attaching the {Project} Infrastructure Subscription] to attach a {Project} subscription to your server.
