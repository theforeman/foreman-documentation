...
[id="creating-image-based-hosts-on-{compute-resource-id}-by-using-web-ui"]
= Creating image-based hosts on {compute-resource} by using {ProjectWebUI}

[role="_abstract"]
In {Project}, you can use {compute-resource} provisioning to create hosts from an existing image.
The new host entry triggers the {compute-resource} server to create the instance using the pre-existing image as a basis for the new volume.
