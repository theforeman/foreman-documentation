...
[role="_abstract"]
You can install, upgrade, or remove packages on a host by using the {Project} API for automation, scripts, or integration with other tools.
...
Limits the number of hosts on which the job is run concurrently.
...
"scheduling":: Optional.
Specifies time boundaries for when to start the package action.
+
--
...
"ssh":: Optional.
Specifies the credentials of an SSH user, if you want to perform the package action as that user.
"dynamic_query":: Optional.
If you supplied the `"scheduling"` object, you can make the search query be evaluated when the job runs by specifying the `"targeting_type/dynamic_query"` field.
+
--
