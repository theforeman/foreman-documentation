...
====
* On the *Advanced* tab, you can enable container and flatpak registry access without a username or password by selecting the *Set up container registry certs* checkbox. This creates links from the certificates created by Subscription Manager and places them in the `/etc/containers/certs.d` directory.
