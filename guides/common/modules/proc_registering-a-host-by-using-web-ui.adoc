...
====
* On the *Advanced* tab, you can enable container and flatpak registry access without a username or password by selecting the *Set up container registry certs* checkbox. This creates links from the entitlement certificates created by Subscription Manager and places them in the `/etc/containers/certs.d` directory. Using certificate based authentication also enforces lifecycle management of container and flatpak content for the host.
