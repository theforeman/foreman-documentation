...
* You must configure {Project} to use this IPv4 HTTP proxy server as the default proxy. For more information, see {InstallingProjectDocURL}performing-additional-configuration#adding-a-default-http-proxy_{project-context}[Adding a Default HTTP Proxy to Satellite].
