[id="configuring-dns-dhcp-and-tftp-on-projectserver_{context}"]
= Configuring DNS, DHCP, and TFTP on {ProjectServer}

To configure the DNS, DHCP, and TFTP services on {ProjectServer}, use the `{foreman-installer}` command with the options appropriate for your environment.
