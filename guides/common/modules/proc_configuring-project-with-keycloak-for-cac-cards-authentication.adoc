[id="configure-project-with-keycloak-for-cac-cards-authentication_{context}"]
= Configuring {Project} with {Keycloak} for CAC Cards Authentication

Use this procedure to configure {Project} to use {Keycloak} as an OpenID provider for external authentication with CAC cards.
