...
[id="adding-an-amazon-ec2-connection-to-{project-context}-server-by-using-web-ui"]
= Adding an Amazon EC2 connection to {ProjectServer} by using {ProjectWebUI}

[role="_abstract"]
Add Amazon EC2 as a compute resource to provision and manage hosts on EC2.
...
For more information, see xref:http-proxy-for-communication-with-{compute-resource-id}[].
