...
After you install the Infoblox modules on {SmartProxy} and set up the view by using the `{foreman-installer}` command, you cannot edit the view.

{SmartProxyServer} communicates with a single Infoblox node by using the standard HTTPS web API.
