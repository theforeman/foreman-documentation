...
[NOTE]
====
Assigning a {SmartProxy} to the same location as your {ProjectServer} with an embedded {SmartProxy} prevents Red{nbsp}Hat Lightspeed from uploading the Red{nbsp}Hat Lightspeed inventory.
To enable the inventory upload, synchronize SSH keys for both {SmartProxies}.
====
endif::[]

ifndef::satellite[]
Assigning a {SmartProxy} to the same location as your {ProjectServer} with an embedded {SmartProxy} prevents Red{nbsp}Hat Insights from uploading the Insights inventory.
To enable the inventory upload, synchronize SSH keys for both {SmartProxies}.
endif::[]
