...
[id="associating-external-users-with-user-groups"]
= Associating external users with user groups
...
For more information, see xref:configuring-an-ldap-server-as-an-external-identity-provider-for-{project-context}[].
+
