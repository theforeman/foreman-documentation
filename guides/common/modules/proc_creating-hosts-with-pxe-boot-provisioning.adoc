...
PXE boot provisioning is the simplest method of host provisioning.
This method requires minimal interaction during the process.
...
When you start the host, the host detects the DHCP service, receives the HTTP endpoint of the installation content, and installs the operating system.

ifdef::katello,satellite,orcharhino[]
When the installation completes, the host also registers to {ProjectServer} by using the activation key, installs the necessary host management tools from the {project-client-name} repository, and performs initial configuration.
endif::[]
ifndef::katello,satellite,orcharhino[]
When the installation completes, the host also registers to {ProjectServer} and performs initial configuration.
endif::[]
