...
* Import the CA certificate that is set as `websockets_ssl_cert`.
Adding a security exception in the browser is not enough for using noVNC.
You can get the path by running `{foreman-installer} --full-help | grep -- "--foreman-websockets-ssl-cert\|--foreman-server-ssl-cert"`.
endif::[]
