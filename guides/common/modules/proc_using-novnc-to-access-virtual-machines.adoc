...
* You must import the Katello root CA certificate into your {ProjectServer}. Adding a security exception in the browser is not enough for using noVNC. For more information, see the {AdministeringDocURL}sect-Red_Hat_Satellite-Administering_Red_Hat_Satellite-Installing_the_Katello_Root_CA_Certificate[Installing the Katello Root CA Certificate] section in the _Administering {ProjectName}_ guide.
