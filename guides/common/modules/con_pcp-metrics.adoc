...
[id="retrieving-and-analyzing-performance-metrics_{context}"]
= Retrieving and analyzing performance metrics

[role="_abstract"]
You can monitor {Project} performance using PCP metrics, which provide system-level data (CPU, memory, network) and {Project}-specific data (processes, PostgreSQL, Rails) in a structured format.
Understanding PCP metrics helps you identify performance bottlenecks and optimize your deployment.

Metrics are stored in a tree-like structure.
For example, all network metrics are stored in a node named `network`.
Each metric can be a single value, or a list of values, known as instances.
For example, kernel load has three instances, the 1-minute, 5-minute, and 15-minute average.
