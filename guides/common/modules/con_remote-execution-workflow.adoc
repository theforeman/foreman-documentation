...
[role="_abstract"]
When you run a remote job on hosts, {Project} uses various strategies to select a remote execution {SmartProxy} for each host to balance the load across available {SmartProxies}.
