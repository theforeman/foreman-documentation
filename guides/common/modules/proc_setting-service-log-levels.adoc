...
[NOTE]
====
Each service inherits the default value from the `log-level` parameter, unless you set the value for the service by using the `<service>-log-level` parameter.
Running `foremanctl deploy --log-level <new-value>` or `foremanctl deploy --reset-log-level` does not affect the `--<service>-log-level` parameters.
====
