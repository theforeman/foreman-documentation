:_mod-docs-content-type: PROCEDURE

[id="setting-service-log-levels"]
= Setting service log levels

[role="_abstract"]
You can set the log level for {Project} services by using `{foremanctl} deploy` parameters.

At the initial deployment, when the containers are first created, the log level is set to the default value of `info`.
You can change the log level for all services or for individual services.

.Procedure
* To set the global log level for all supported services, run:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# {foremanctl} deploy --log-level _Log_Level_
----
+
For example, to set the global log level to `debug`, run:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# {foremanctl} deploy --log-level debug
----

* To override the log level for a specific service, use a service-specific parameter, such as `--foreman-log-level` and `--foreman-proxy-log-level`.
+
For example, to set only Foreman to `error` while keeping other services at the global level, run:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# {foremanctl} deploy --foreman-log-level error
----
+
After you complete debugging, reset the log level to the default to avoid filling up host disk space:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# {foremanctl} deploy --reset-foreman-log-level
----
+
[NOTE]
====
Each service inherits the default value from the `log-level` parameter, unless you set the value for the service by using the `_My_Service_-log-level` parameter.
Running `foremanctl deploy --log-level _New_Log_Level_` or `foremanctl deploy --reset-log-level` does not affect the `--_My_Service_-log-level` parameters.
====

.Verification
* Verify that a new log level took effect as actions occur:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# journalctl --unit foreman --follow
----

.Additional resources
* xref:log-level-parameters[]
