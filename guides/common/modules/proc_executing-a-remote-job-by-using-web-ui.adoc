...
[id="executing-a-remote-job-by-using-web-ui"]
= Executing a remote job by using {ProjectWebUI}

[role="_abstract"]
You can execute a job that is based on a job template against one or more hosts from the {ProjectWebUI}.
...
* Remote execution is installed and configured on {Project}.
