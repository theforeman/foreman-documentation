...
= Configuring {Project} settings for {Keycloak-RHBK} authentication using the CLI

Use this procedure to configure {Project} settings for {Keycloak-RHBK} authentication using the {Project} CLI.
...
* Ensure that the *Client authentication* setting in the {Project} client in the {Keycloak-RHBK} web UI is set to OFF.
...
. Set the algorithm for encoding on {Keycloak-RHBK}, for example, `RS256`:
+
----
...
[NOTE]
====
If you register several {Keycloak-RHBK} clients to {Project}, ensure that you append all audiences in the array.
For example:
[options="nowrap", subs="+quotes,attributes"]
----
# hammer settings set --name oidc_audience \
...
. Retrieve the ID of the {Keycloak-RHBK} authentication source:
+
----
