[id="Updating-Disconnected-{project-context}-on-EL8-EUS_{context}"]
= Updating a disconnected {ProjectServer} on {EL} 8 using EUS

You can update your disconnected {Project} on {RHEL} 8 by synchronizing the required Extended Update Support repositories on the connected {Project} and syncing the content to the disconnected {Project} using `reposync`.
...
* {RepoRHEL8BaseOS}
* {RepoRHEL8AppStream}
* {RepoRHEL8EUSServerSatelliteServerProjectVersion} 
* {RepoRHEL8EUSServerSatelliteMaintenanceProjectVersion}
...
baseurl=_https://{foreman-example-com}_/pulp/content/_My_Organization_/Library/content/eus/rhel8/x86_64/satellite/{ProjectVersion}/os
enabled = 1
sslclientcert = /etc/pki/katello/certs/org-debug-cert.pem
sslclientkey = /etc/pki/katello/certs/org-debug-cert.pem
sslcacert = /etc/pki/katello/certs/katello-server-ca.crt
...
baseurl=_https://{foreman-example-com}_/pulp/content/_My_Organization_/Library/content/eus/rhel8/x86_64/sat-maintenance/{ProjectVersion}/os
enabled = 1
sslclientcert = /etc/pki/katello/certs/org-debug-cert.pem
sslclientkey = /etc/pki/katello/certs/org-debug-cert.pem
sslcacert = /etc/pki/katello/certs/katello-server-ca.crt
...
----
. Use the `reposync` command to download the contents of the repositories from the connected {ProjectServer}:
+
[options="nowrap" subs="+quotes,verbatim,attributes"]
----
# dnf reposync \
...
--repoid {RepoRHEL8EUSServerSatelliteMaintenanceProjectVersion} \
--repoid {RepoRHEL8EUSServerSatelliteServerProjectVersion} \
-n \
-p ~/{Project}-repos
----
+
...
baseurl=file:///root/{Project}-repos/{RepoRHEL8EUSServerSatelliteServerProjectVersion}
enabled = 1

[{RepoRHEL8EUSServerSatelliteMaintenanceProjectVersion}]
name={ProjectName} Maintenance 6 for RHEL 8 Server RPMs x86_64
baseurl=file:///root/{Project}-repos/{RepoRHEL8EUSServerSatelliteMaintenanceProjectVersion}
enabled = 1
----
