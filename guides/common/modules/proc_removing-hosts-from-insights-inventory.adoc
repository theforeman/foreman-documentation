...
[NOTE]
====
Automatic mismatch deletion runs once a day as a scheduled task.
After removing hosts from {Project}, hosts are deleted from the {RHCloud} inventory during the next scheduled sync.
Clicking *Sync all inventory status* updates the status of hosts but does not trigger immediate deletion.
====
