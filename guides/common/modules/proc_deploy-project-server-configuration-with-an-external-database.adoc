...
For more information, see {AdministeringDocURL}installing-postgresql-as-an-external-database_admin[Installing PostgreSQL as an external database] in _{AdministeringDocTitle}_.
...
----
+
You can enable one or more features by adding the `--add-feature _Feature-1_ --add-feature _Feature-2_` option to the `{foremanctl} deploy` command.
