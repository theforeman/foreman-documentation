...
+
If you set a wildcard value `*` for the certificate's Name `CN =` in the `/root/{context}_cert/openssl.cnf` configuration file, you must add `-t {smart-proxy-context}` to the `katello-certs-check` command.
+
