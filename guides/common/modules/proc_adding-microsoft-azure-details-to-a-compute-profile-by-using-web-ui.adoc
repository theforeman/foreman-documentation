...
[id="adding-microsoft-azure-details-to-a-compute-profile-by-using-web-ui"]
= Adding Microsoft Azure details to a compute profile by using {ProjectWebUI}

[role="_abstract"]
You can create custom compute profiles to bundle {compute-resource} hardware settings, for example, CPU cores, memory, and storage details.
