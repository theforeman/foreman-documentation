...
ifdef::vmware-el,nutanix-el,hyperv-el[]
.Prerequisites
* You have registered {the-target} to {ProjectServer}. For more information, see {ProvisioningDocURL}registering_hosts_to_server_managing-hosts#Registering_Hosts_by_Using_Global_Registration_managing-hosts[Registering hosts by using global registration] in _{ProvisioningDocTitle}_.
endif::[]
