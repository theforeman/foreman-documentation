...
.Procedure
. Copy the image to an existing HTTP server which installed hosts can reach.
...
. In the *Value* field, enter the URL of the image location, such as `https://_webserver.example.com_/_path_/_My_Image_Name_.tar`.
