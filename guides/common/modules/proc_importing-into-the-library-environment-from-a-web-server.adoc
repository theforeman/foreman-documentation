:_mod-docs-content-type: PROCEDURE

[id="importing-into-the-library-environment-from-a-web-server"]
= Importing into the Library environment from a web server

[role="_abstract"]
You can import exported Library content directly from a web server into the Library environment on a downstream {ProjectServer} when exports are in syncable format.

.Prerequisites
* The exported files must be in the syncable format.
* The exported files must be accessible through HTTP/HTTPS.
ifdef::katello,satellite,red_hat_enterprise_linux[]
* If there are any Red{nbsp}Hat repositories in the exported content, the subscription manifest of the importing organization must contain subscriptions for the products contained within the export.
endif::[]
* The user importing the content has the _Content Importer_ role.

.Procedure
. Identify the Organization that you want to import into.
. To import the Library content to {ProjectServer}, enter the following command:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
$ hammer content-import library \
--organization="_My_Organization_" \
--path=http://_{server-example-com}_/pub/exports/2021-02-25T21-15-22-00-00/
----
+
A new content view called `Import-Library` is created in the target organization.
This content view is used to facilitate the Library content import.
+
By default, this content view is not shown in the {ProjectWebUI}.
`Import-Library` is not meant to be assigned directly to hosts.
Instead, assign your hosts to `Default Organization View` or another content view.
