...
[id="adding-{compute-resource-id}-images-to-{project-context}-by-using-web-ui"]
= Adding {compute-resource} images to {Project} by using {ProjectWebUI}
...
. Click the name of the {compute-resource} connection.
. Click *Create Image*.
...
ifdef::openstack-provisioning,rhv-provisioning,gce-provisioning[. From the *Image* list, select an image from the {compute-resource} compute resource.]
