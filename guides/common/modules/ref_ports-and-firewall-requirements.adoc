...
ifdef::katello,satellite,orcharhino[]
| 5646 | TCP | AMQP | {ProjectServer} |
| 5671 |  |  | Remote install | Send install command to client
| 5671 |  |  | Dispatch router (hub) | Remote install | Forward message to dispatch router on {Project}
| 5671 | | | {ProjectServer} | Send install command to client
| 5671 | | | {ProjectServer} | Forward message to dispatch router on {Project}
endif::[]
