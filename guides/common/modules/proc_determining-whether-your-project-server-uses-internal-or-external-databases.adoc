...
.Procedure
ifdef::containerized[]
* On {ProjectServer}, inspect the status of the local PostgreSQL container service:
+
...
During a migration to external databases, the local `postgresql.service` unit is still active until you remove the internal PostgreSQL deployment.
endif::[]
