...
.Prerequisites
* Your {ProjectServer} must have the same hostname, configuration, and be the same minor version (X.Y) as the original system.
If the original system that experienced a failure is unavailable, provision a system with the same configuration and hostname.
...
ifdef::containerized[]
. Install {ProjectServer} or {SmartProxyServer} on the system where you want to restore the backup.
For more information, see {InstallingServerDocURL}[{InstallingServerDocTitle}] and {InstallingSmartProxyDocURL}[{InstallingSmartProxyDocTitle}].
endif::[]
ifndef::containerized[]
...
. Copy the backup data to the local file system on your {ProjectServer} or {SmartProxyServer}.
This procedure uses _/var/backup_directory_.
ifdef::containerized[]
