...
If you are configuring a client that will provide Hammer CLI authentication, configure the {Project} client and add {keycloak} as an external authentication source.

.Prerequisites

* For {keycloak-quarkus}: Ensure that the *Client authentication* setting in the {Project} client in the {keycloak-quarkus} web UI is set to `OFF`.
See also xref:configuring-the-{project-context}-client-in-keycloak_keycloak-quarkus[].
+
For {keycloak-wildfly}: Ensure that the *Access Type* setting in the {Project} client in the {keycloak-wildfly} web UI is set to `public`.
