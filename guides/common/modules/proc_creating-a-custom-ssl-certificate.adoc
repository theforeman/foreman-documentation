...
----
# openssl req -new \
-key _/root/{context}_cert/{context}_cert_key.pem_ \ <5>
-config _/root/{context}_cert/openssl.cnf_ \ <6>
-out _/root/{context}_cert/{context}_cert_csr.pem_ <7>
----
<5> Path to the private key
<6> Path to the configuration file
<7> Path to the CSR to generate
