...
[id="multiple-manifest-scenario"]
= Multiple manifest scenario

[role="_abstract"]
You can use many Red{nbsp}Hat{nbsp}Network subscription manifests in one {ProjectServer} to manage systems from different Red{nbsp}Hat{nbsp}Network accounts or entities.
This allows you to assign subscriptions to separate organizations and manage those organizations independently.
...
image::planning-for-foreman-server-multiple-manifests.svg[{ProjectServer} with multiple manifests]
endif::[]
ifdef::orcharhino[]
image::planning-for-foreman-server-multiple-manifests-orcharhino.svg[{ProjectServer} with multiple manifests]
endif::[]
ifdef::satellite[]
...
If you delete the manifest from the Red Hat Customer Portal or in the {ProjectWebUI} it will unregister all of your hosts.

ifdef::satellite[]
.Additional resources
* https://www.redhat.com/en/technologies/management/satellite[{Project} Add-On]
endif::[]
