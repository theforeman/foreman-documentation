...
[id="host-registration-methods"]
= Host registration methods

[role="_abstract"]
ifdef::orcharhino[]
You can register hosts to {Project} using global registration.
endif::[]
ifndef::orcharhino[]
You can register hosts to {Project} using different methods.
Choose the method that fits your environment, for example global registration for most hosts or the bootstrap script for legacy workflows.
endif::[]
...
For more information, see xref:monitoring-hosts-by-using-{insights-id}-in-cloud[].
