...
[role="_abstract"]
Increase {insights-iop} logging to `DEBUG` to capture detailed diagnostic output when troubleshooting {Insights} operations on your {ProjectServer}.
...
loggers:
  ...
  "":
    level: "DEBUG"
----
.. Replace the `iop-core-engine-config-yml` Podman secret and restart the IOP core engine service:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
