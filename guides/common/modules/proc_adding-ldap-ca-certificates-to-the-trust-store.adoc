...
[id="adding-ldap-ca-certificates-to-the-trust-store"]
= Adding LDAP CA certificates to the trust store
...
You can add the certificate to the system trust store on your {ProjectServer}, or associate a CA certificate directly with the LDAP authentication source.

[NOTE]
====
You can skip this procedure and set the CA certificate when configuring LDAP, without adding them to the system trust store.
For more information, see xref:configuring-project-to-use-ldap[].
====
...
ifdef::containerized[]
* The LDAP server's TLS certificate chains to a publicly trusted root CA present in the container's base image.
If the LDAP server uses a certificate signed by a private or internal CA, such as Active Directory Certificate Services or an internal PKI, you must paste the CA certificate into the *CA certificate* field.
endif::[]
