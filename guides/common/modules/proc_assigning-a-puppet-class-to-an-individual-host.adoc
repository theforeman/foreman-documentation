...
[role="_abstract"]
Assigning a Puppet class to an individual host enables you to apply a specific configuration to that host, overriding any settings inherited from its host group.
This is useful for custom configurations that you want to apply to a single host.
...
[TIP]
====
You can rerun the Puppet agent on your host:
...
. Check which ntp servers are used for clock synchronization:
+
[options="nowrap", subs="verbatim,quotes,attributes"]
----
# cat /etc/ntp.conf
