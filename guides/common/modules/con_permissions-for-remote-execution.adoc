...
If you adjust the filter with the `view_job_templates` permission, the users with this can only see and trigger jobs based on matching job templates.
...
Those with one of these roles or with a custom role with the `execute_jobs_on_infrastructure_hosts` permission can execute remote jobs against registered {ProjectName} and {SmartProxy} hosts.
...
The first line in this example permits you to apply the *Reboot* template to one selected host.
