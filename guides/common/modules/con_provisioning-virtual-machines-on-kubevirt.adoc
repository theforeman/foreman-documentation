...
* You must have the `get`, `delete`, `create`, `update` ,`patch` , `list`, and `watch` permissions for VMs and other {KubeVirt} resources for the {Kubernetes} cluster in the specified namespace. To get these permissions, your user must have the `kubevirt.io:edit` or `kubevirt.io:admin` ClusterRole bound with a RoleBinding.
* To use auto-completion for network selection, you also need the `get` and `list` permissions for `network-attachment-definitions.k8s.cni.cncf.io`.
