...
[id="recovering-from-a-full-disk"]
= Recovering from a full disk

[role="_abstract"]
If a logical volume (LV) with the Pulp database on it is full, Pulp tasks can fail due to insufficient space.
You can recover from the situation by increasing the amount of free space available on the LV.
...
ifdef::containerized[]
ifndef::orcharhino[]
For more information, see {RHELDocsBaseURL}10/html-single/configuring_and_managing_logical_volumes/index#growing-a-logical-volume-and-file-system_modifying-the-size-of-a-logical-volume[Growing a logical volume and file system] in _{RHEL}{nbsp}10 Configuring and managing logical volumes_.
endif::[]
endif::[]
ifdef::containerized[]
ifdef::katello[]
