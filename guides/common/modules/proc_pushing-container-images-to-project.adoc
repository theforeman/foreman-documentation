:_mod-docs-content-type: PROCEDURE

[id="pushing-container-images-to-{project-context}"]
= Pushing container images to {Project}

[role="_abstract"]
Push container images to the {Project} container registry to distribute custom container images.
You can use Podman or Docker to consume container images on hosts.

.Prerequisites
* Review xref:considerations-for-using-the-{project-context}-container-registry[].
* To push content to {Project}, ensure your {Project} account has the `edit_products` permission.
* Ensure that a product exists before pushing a repository.
For more information, see xref:creating-a-custom-product-by-using-web-ui[].
* Your {Project} account has a role that grants the `create_personal_access_tokens` permission to generate an access token for authentication.
{Project} automatically generates a token when you run `podman login`.

.Procedure
. Log in to the container registry:
+
[options="nowrap", subs="+quotes,attributes"]
----
# podman login _{foreman-example-com}_
----
. Push a container image to the {Project} container registry.
To indicate which organization, product, and repository the container image belongs to, include the organization and product in the container registry name.
You can address the container destination by using one of the following schemas:
+
[options="nowrap", subs="+quotes,attributes"]
----
$ podman push _My_Container_Image_Hash_ _{foreman-example-com}_/_My_Organization_Label_/_My_Product_Label_/_My_Repository_Name_[:__My_Tag__]
$ podman push _My_Container_Image_Hash_ _{foreman-example-com}_/id/_My_Organization_ID_/_My_Product_ID_/_My_Repository_Name_[:__My_Tag__]
----
+
After the content push has completed, a repository is created in {Project}.
