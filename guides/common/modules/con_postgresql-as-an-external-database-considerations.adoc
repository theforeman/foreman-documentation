...
* Change the database setting when installing {Project}. For details, see xref:Configuring_Installation_{project-context}[Configuring {Project} installation]

* Migrate to the external database on an existing {Project} deployment
link TBA Migrating from internal {Project} databases to external databases