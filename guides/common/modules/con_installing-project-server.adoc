...
You can deploy {ProjectServer} with a default internal database or with an external database.
...
After you install {ProjectServer}, you can add {SmartProxyServers} in your disconnected environment by installing them from an ISO image.
For more information, see {InstallingSmartProxyDocURL}installing-{smart-proxy-context}-server-from-iso[Installing {SmartProxyServer} from ISO] in _{InstallingSmartProxyDocTitle}_.
