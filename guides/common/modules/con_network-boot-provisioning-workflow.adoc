...
[id="network-boot-provisioning-workflow"]
= Network boot provisioning workflow

[role="_abstract"]
The {Project} provisioning process follows a basic PXE workflow.
It might be useful to understand how {Project} coordinates network services and content delivery to install the system for troubleshooting, locating provisioning failures, and ensuring a successful host provisioning.
