...
. If you configured the host to use any Puppet classes, the host configures itself by using the modules.
...
After you submit a new host request, you must boot the specific host with the boot disk that you download from {Project} and transfer by using a USB port of the host.
