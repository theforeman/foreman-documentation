...
= Configuring {Project} settings for {Keycloak-SSO} authentication using the web UI

Use this procedure to configure {Project} settings for {Keycloak-SSO} authentication using the {ProjectWebUI}.
...
* Ensure that the *Access Type* setting in the {Project} client in the {Keycloak-SSO} web UI is set to *confidential*.
...
. Locate the *OIDC Algorithm* row, and in the *Value* column, set the algorithm for encoding on {Keycloak-SSO} to *RS256*.
. Locate the *OIDC Audience* row, and in the *Value* column, set the value to the client ID for {Keycloak-SSO}.
...
. Click the *Locations* tab and add locations that can use the {Keycloak-SSO} authentication source.
. Click the *Organizations* tab and add organizations that can use the {Keycloak-SSO} authentication source.
. Click *Submit*.
