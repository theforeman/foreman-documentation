...
| *OAuth map users* | No | When enabled, {Project} maps users by username in the request-header.
...
| *Authorize login delegation auth source user autocreate* | External | Name of the external auth source where unknown externally authentication users (see *Authorize login delegation*) are created.
