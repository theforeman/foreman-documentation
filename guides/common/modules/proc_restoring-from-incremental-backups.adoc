...
[role="_abstract"]
Restore {Project} or {SmartProxyServer} from an incremental backup to return the system to the state at the time of the backup.
...
.Next steps
* If you create a new instance of the {ProjectServer} or {SmartProxyServer}, decommission the old instance after restoring the backup.
