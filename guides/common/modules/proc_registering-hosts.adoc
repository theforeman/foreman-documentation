...
ifdef::katello[]
. From the *REX pull mode* list, select whether you want to deploy {Project} remote execution pull client or not.
+
If set to `Yes`, {Project} remote execution pull client will be installed on the registered host.
The inherited value is based on the `host_registration_remote_execution_pull` parameter.
It can be inherited, for example from a host group, an operating system, or an organization.
When overridden, the selected value will be stored on host parameter level.

The registered host has to have access to a repository containing `foreman_ygg_worker` package.
ifndef::satellite[]
