:_mod-docs-content-type: CONCEPT

[id="monitoring-hosts-by-using-{insights-iop-id}"]
= Monitoring hosts by using {insights-iop}

[role="_abstract"]
You can use local services of {insights-iop} to assess system health on hosts running {RHEL} without connecting to the {RHCloud}.
{insights-iop} provides advisor recommendations and vulnerability reports.

If you use {insights-iop}:

* {insights-iop} services run locally on your {ProjectServer}.
* You cannot use any of the hosted services on the {RHCloud}.
* {Project} only communicates with the locally running services of {insights-iop}.
* {Project} does not send any reports to the {RHCloud}.
* {Project} does not forward any requests through the Insights client to the {RHCloud}.

.Prerequisites
* You have installed {insights-iop} on your {ProjectServer}.
For more information, see {InstallingServerDisconnectedDocURL}installing-and-configuring-{insights-iop-id}[Installing and configuring {insights-iop}] in _{InstallingServerDisconnectedDocTitle}_.
* For new {Project} hosts, install and configure the Insights client during host registration to {Project}.
For more information, see xref:Registering_Hosts_by_Using_Global_Registration_managing-hosts[].
* For hosts already registered to {Project}, install and configure the Insights client on your hosts by using an Ansible role.
For more information, see xref:deploying-insights-client-on-registered-hosts_{context}[].
* You can change the default schedule for running the Insights client by configuring `insights-client.timer` on a host.
ifdef::satellite[]
For more information, see {RHDocsBaseURL}red_hat_lightspeed/1-latest/html/client_configuration_guide_for_red_hat_lightspeed/assembly-client-changing-schedule[Changing the insights-client schedule] in the _Client Configuration Guide for Red Hat Lightspeed_.
endif::[]

.Troubleshooting
* You can find the logs in the `/var/log/foreman/production.log` file.
