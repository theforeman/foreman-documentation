...
* Alternatively, if you want to configure certificate authentication for container and flatpak content on a registered host:
- Run the *Flatpak - Login to registry via podman* job template on the host.
- Set the *Set up certificate authentication* input to `true` and enter the URL of the {Project} or {SmartProxy} server as the registry URL.
