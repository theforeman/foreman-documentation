...
For details on using Ansible Vault see [Encrypting content with Ansible Vault](https://docs.ansible.com/projects/ansible/latest/vault_guide/vault_encrypting_content.html).

ifndef::containerized[]
The procedure below assumes that you have a role `_Role_Name_` which has its main variables file encrypted using Ansible Vault and that you placed the role under `/etc/ansible/roles`.
endif::[]
