...
[role="_abstract"]
Implement multiple lifecycle environment paths to create sequential content progression stages from development through production.
Lifecycle environment best practices enable controlled content deployment and version management.
...
All hosts consuming this content view or composite content view are able to install packages from the _Production_ lifecycle environment.
Note that these packages are not installed or updated automatically.
** If you encounter errors during patching hosts, attach the host to a previous version of the content view.
...
** When patching hosts, change the lifecycle environment from `2023-Q4` to `2024-Q1` using the {ProjectWebUI}, {Project} API, Hammer CLI, or an activation key.
