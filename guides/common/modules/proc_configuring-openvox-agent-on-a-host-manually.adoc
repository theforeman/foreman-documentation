...
= Configuring OpenVox agent on a hostmanually
...
ifdef::orcharhino[]
* Ensure a repository containing the OpenVox agent is enabled on the host, for example {client-puppet-repo-url}[Puppet agent for {client-os}].
endif::[]
...
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# {client-package-install-el7} openvox-agent
----
endif::[]
endif::[]
...
. Add the Puppet OpenVox agent to `PATH` in your current shell using the following script:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
