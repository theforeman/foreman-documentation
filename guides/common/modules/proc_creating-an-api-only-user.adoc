...
[role="_abstract"]
You can create a user account that can interact only with the {Project} API.
This is useful for service accounts or integrations that must not use the {ProjectWebUI}.
...
For more information, see xref:creating-a-personal-access-token[].
