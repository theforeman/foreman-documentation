...
This helps you keep hosts on a supported release and can be run across multiple hosts by using remote execution.

{Team} supports the following upgrade paths:
...
.Prerequisites
* Your {EL} hosts meet the requirements for the upgrade.
ifndef::orcharhino[]
...
* Your {EL} hosts are prepared for the upgrade.
ifndef::orcharhino[]
...
* Remote execution is enabled on {Project}.
For more information, see xref:configuring-and-setting-up-remote-jobs[].
endif::[]
* {Project} SSH keys are distributed to the hosts that you want to upgrade.
