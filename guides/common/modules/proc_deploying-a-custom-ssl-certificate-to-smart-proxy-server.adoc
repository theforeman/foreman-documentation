...
[role="_abstract"]
If your {ProjectServer} is configured with a custom certificate, you must configure your {SmartProxyServer} with a custom SSL certificate signed by the same Certificate Authority.
...
--certificate-server-certificate "_/root/{smart-proxy-context}_cert/{smart-proxy-context}_cert.pem_" \
--certificate-server-key "_/root/{smart-proxy-context}_cert/{smart-proxy-context}_cert_key.pem_"
----
+
--
...
`--certificate-server-certificate`:: Path to {SmartProxyServer} certificate file that is signed by a Certificate Authority.
`--certificate-server-key`:: Path to the private key for the {SmartProxyServer} certificate.
--
+
...
After you configure {SmartProxyServer} to use a custom SSL certificate, you must refresh the self-signed CA certificate on registered hosts.
