...
* Read the {ProjectName} {ProjectVersion} {ReleaseNotesDocTitle}.
...
For information about changes in these tools, see the {ProjectName} {ProjectVersion} {ReleaseNotesDocTitle}.
ifdef::satellite[]
* If you use Puppet to manage your host configuration, ensure your Puppet modules are compatible with OpenVox 8, which uses Ruby 3.2.
...
* The upgrade process shuts down {Project} services.
Ensure to plan for the required downtime.
The upgrade process duration varies depending on your hardware configuration, network speed, and the amount of data on the server:
...
* {ReleaseNotesDocURL}[{ReleaseNotesDocTitle}]
ifdef::satellite[]
* {AdministeringDocURL}cloning-{project-context}-server[Cloning {ProjectServer} in _{AdministeringDocTitle}_]
endif::[]
