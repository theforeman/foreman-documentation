...
[role="_abstract"]
You can register hosts to {Project} by generating a `curl` or `wget` registration command on {Project} and running this command on an unlimited number of hosts by using provisioning templates over the {Project} API.
...
For example, you can deploy {Project} SSH keys to hosts during registration to {Project} to enable remote execution jobs, configure hosts with {Insights} during registration to {Project}, or install the OpenVox agent on hosts to enable configuration management with Puppet.
