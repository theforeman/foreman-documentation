...
+
* *Product Variant* is set to *{RHEL}*.
...
. On the *Product Software* tab, download the Binary DVD image for the latest {RHEL} version.
...
. Copy the ISO files to `/var/tmp` on the {Project} base system or other accessible storage device.
+
[options="nowrap"]
----
...
. Ensure the ISO images for {RHEL} and {ProjectName} are mounted:
+
[options="nowrap"]
----
# findmnt -t iso9660
...
----
+
If you have successfully installed the {Project} packages, the following message is displayed: `Install is complete. Please run satellite-installer --scenario satellite`.
