...
[role="_abstract"]
ifdef::satellite[]
Download policies control when {Project} stores RPM packages during synchronization versus fetching them on request from {SmartProxies} or clients.
endif::[]
ifndef::satellite[]
Download policies control when {Project} stores Deb, Yum, and container image content during synchronization versus fetching it on request from {SmartProxies} or clients.
endif::[]
