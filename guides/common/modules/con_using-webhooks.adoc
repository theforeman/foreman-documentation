...
Where it is necessary to run additional commands or edit files, the shellhooks plug-in for {SmartProxies} is available.
The shellhooks plug-in enables you to define a shell script on the {SmartProxy} that can be executed through the API.

You can use webhooks successfully without installing the shellhooks plug-in.
