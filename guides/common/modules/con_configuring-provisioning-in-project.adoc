...
[id="configuring-provisioning-in-{project-context}"]
= Configuring provisioning in {Project}
...
The provisioning process depends on whether you want to provision bare-metal hosts, virtual machines, or cloud instances, but it includes defining installation media, configuring provisioning templates, and other tasks.
...
ifndef::satellite[]
* {ProvisioningDocURL}installing-the-discovery-service[Installing the Discovery service in _{ProvisioningDocTitle}_]
endif::[]
* {ProvisioningDocURL}[_{ProvisioningDocTitle}_]
