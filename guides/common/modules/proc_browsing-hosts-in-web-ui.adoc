...
.Procedure
. Search for a host: Type in the *Search* field, and use an asterisk ({asterisk}) to perform a partial string search.
For example, if searching for a host named `{server-example-com}`, click the *Content Hosts* page and type `server*` in the *Search* field.
...
====
. Navigate between hosts: Click *⇄* next to the hostname.
