...
[id="authenticating-hosts-with-ssh-certificates"]
= Authenticating hosts with SSH certificates

[role="_abstract"]
Your {SmartProxy} can use SSH host certificates to verify host identity.
...
[NOTE]
====
You must use the `-h` option when generating the SSH host certificate with `ssh-keygen`.
====
+
[ADMONITION]
====
...
.. If the host CA is not located on the {SmartProxyServer}, copy the host CA public key file to the {SmartProxyServer}.
