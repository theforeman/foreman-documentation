...
[role="_abstract"]
Red{nbsp}Hat repositories sync automatically over {ISS} network sync, but you must configure each custom repository on the downstream {ProjectServer} to synchronize from its published URL on the upstream server.
