...
[id="installing-errata-on-hosts-by-using-mcp"]
= Installing errata on hosts by using MCP

[role="_abstract"]
You can prompt your AI application to create remote execution jobs in {Project} to install installable errata on hosts.
...
. The {Project} MCP server provides a prompt named `Apply Errata to Hosts`, which you can use to install errata on the hosts.
