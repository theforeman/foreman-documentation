...
| *Fallback to Any Capsule* | No | Search the host for any proxy with Remote Execution, useful when the host has no subnet or the subnet does not have an execution proxy.
...
| *refer IPv6 over IPv4* | No | When connecting using IP address, is the IPv6 addresses preferred?
...
| *Cockpit URL* | Empty | Where to find the Cockpit instance for the Web Console button.
