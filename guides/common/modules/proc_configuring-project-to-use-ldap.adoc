...
[id="configuring-{project-context}-to-use-ldap"]
...
In the *CA certificate* field, paste one or more PEM-encoded CA certificates to verify the TLS certificate of the LDAP server.
ifndef::containerized[]
If you leave this field blank, {Project} uses the system truststore.
If you set a CA certificate, only the configured certificates are trusted and the system truststore is not consulted.
endif::[]
. On the *Account* tab, enter the account information and domain name details.
For more information, see the following sections:
** xref:example-settings-for-ldap-connections[]
** xref:example-ldap-filters[]
...
For more information, see {AdministeringDocURL}Managing_Users_and_Roles_admin[Managing users and roles] in _{AdministeringDocTitle}_.
...
For more information about assigning appropriate roles to user accounts, see {AdministeringDocURL}assigning-roles-to-a-user-by-using-cli[Assigning roles to a user by using Hammer CLI] in _{AdministeringDocTitle}_.
