...
[id="configuring-journald-retention_{context}"]
= Configuring journald log retention and disk space limits

[role="_abstract"]
Because containerized {ProjectServer} routes all service logs through `journald`, you must configure `journald` retention settings to prevent unbounded log growth from consuming disk space on the `/var` partition.
These settings replace the `logrotate` configuration used in traditional {Project} deployments.
...
For more information, see xref:containerized-logging-architecture_{context}[].
...
* For a description of all available `journald` retention parameters, see xref:journald-retention-parameters_{context}[].
ifdef::satellite[]
