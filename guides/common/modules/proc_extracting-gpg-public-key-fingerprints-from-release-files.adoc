...
[role="_abstract"]
Extract GPG fingerprints from signed Debian `Release` files so you can import the matching keys into {Project} and trust upstream Deb metadata before you mirror repositories.
...
For more information, see xref:importing-a-custom-gpg-key-by-using-web-ui[] into {Project}.
