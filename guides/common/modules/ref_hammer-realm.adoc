...
* `--realm-proxy-id xref:hammer-option-details-number[NUMBER]` {endash} Proxy ID to use within this realm
* `--realm-type xref:hammer-option-details-value[VALUE]` {endash} Realm type, e.g. FreeIPA or Active Directory
* `-h`, `--help` {endash} Print help
