...
[id="template-macros-reference"]
= Template macros reference

[role="_abstract"]
You can use these macros in ERB templates to add user inputs, load host data, and perform other tasks when writing provisioning, job, or report templates.
Availability and options vary by template type.

For more information about the macros available for each template type in the {ProjectWebUI}, navigate to the following pages in the {ProjectWebUI}:
