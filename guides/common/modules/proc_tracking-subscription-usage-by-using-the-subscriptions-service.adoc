...
ifndef::satellite[]
.Prerequisites
* The `foreman_rh_cloud` plugin is enabled on {ProjectServer}.
You can enable the plugin by running `{foreman-installer}`:
+
...
.Procedure
. On the connected {ProjectServer}, in the {ProjectWebUI}, navigate to *{Insights}* > *Inventory Upload* to configure the `foreman_rh_cloud` plugin and share inventory information with the {RHCloud}.
...
. On the disconnected {ProjectServer}, export usage data in one of the following ways:
