...
[role="_abstract"]
You can export only Library content that changed since the last export to reduce archive size and system resource use when full Library exports are too large.
...
Organizations that have multiple {client-os} trees can occupy several gigabytes of space on {ProjectServer}.
