...
ifdef::satellite[]
The operating system image is based on {EL} 8 and {EL} 7.
endif::[]
ifdef::orcharhino[]
...
ifdef::satellite[]
* For the following {EL} 8 repositories required to build the Discovery image, change the download policy to *Immediate*.
...
+
** The latest *{EL} 8 (Kickstart)* repository.
For example, *{EL} 8 Kickstart x86_64*.
** *Red Hat Satellite Capsule {ProductVersion} for RHEL 8 RPMs x86_64*.
...
* Synchronize the following {EL} 8 repositories required to build the Discovery image:
+
** The latest *{EL} 8 (Kickstart)* repository.
For example, *{EL} 8 Kickstart x86_64*.
** *Red Hat Satellite Capsule {ProductVersion} for RHEL 8 RPMs x86_64*.
