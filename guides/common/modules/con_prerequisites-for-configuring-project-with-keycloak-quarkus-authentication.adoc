...
[IMPORTANT]
====
Configuring {keycloak-quarkus} authentication requires the `--http-relative-path=/auth` context path to be configured in your {keycloak-quarkus} system setup.
If you want to use a different context path, make manual adjustments after the initialisation with `/auth` or configure the `_foreman-openidc_oidc_keycloak_Foreman_Realm_.conf` file of the HTTPd service manually.
ifndef::orcharhino[]
