...
To configure an external DHCP server to use with {ProductName}, you must install the ISC DHCP Service and Berkeley Internet Name Domain (BIND) utilities packages.
...
.Procedure
. On your external DHCP host, install the ISC DHCP Service and Berkeley Internet Name Domain (BIND) utilities packages:
+
[options="nowrap" subs="+quotes,attributes"]
----
# {server-package-manager} install {dhcp-server-package} {bind-package}
----
...
[options="nowrap" subs="+quotes,attributes"]
----
# {server-package-manager} install {nfs-server-package}
