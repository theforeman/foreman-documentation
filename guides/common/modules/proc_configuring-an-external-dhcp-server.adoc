...
NOTE: If you use dnsmasq as an external DHCP server, enable the `dhcp-no-override` setting. This is required because {Project} creates configuration files on the TFTP server under the `grub2/` subdirectory. If the `dhcp-no-override` setting is disabled, clients fetch the bootloader and its configuration from the root directory, which might cause an error.
