...
To configure an external DHCP server to use with {ProductName}, on a {RHEL} server, you must install the ISC DHCP Service and Berkeley Internet Name Domain (BIND) packages.
...
. On a {RHEL} Server server, install the ISC DHCP Service and Berkeley Internet Name Domain (BIND) packages:
+
