...
[role="_abstract"]
You can configure a virtual interface for a host as a VLAN or an alias.
Use a VLAN to connect to separate network segments on one physical interface, or use an alias to add another IP address to an existing interface.
