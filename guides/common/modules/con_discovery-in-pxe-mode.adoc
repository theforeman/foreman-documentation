:_mod-docs-content-type: CONCEPT

[id="discovery-in-pxe-mode"]
= Discovery in PXE mode

[role="_abstract"]
{Project} provides PXE-based Discovery that uses DHCP and TFTP services.
Discovered hosts queued for installation reboot and provision through PXE.

.Discovery workflow in PXE mode
ifdef::satellite[]
image::discovery-pxe-mode-satellite.png[Discovery workflow in PXE mode]
endif::[]
ifdef::orcharhino[]
image::discovery-pxe-mode-orcharhino.svg[Discovery workflow in PXE mode]
endif::[]
ifndef::satellite,orcharhino[]
image::discovery-pxe-mode.svg[Discovery workflow in PXE mode]
endif::[]
