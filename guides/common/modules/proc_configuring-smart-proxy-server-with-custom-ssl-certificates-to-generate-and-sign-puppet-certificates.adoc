...
.Prerequisites
include::snip_prerequisites-configuring-smart-proxy-servers-for-load-balancing.adoc[]
...
--puppet-server-ca "true"
----
. On {SmartProxyServer} that is the Puppetserver Certificate Authority, stop the OpenVox server:
+
