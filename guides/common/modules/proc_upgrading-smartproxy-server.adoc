...
----
+
. If you have Extended Update Support (EUS), synchronize `{RepoRHEL8EUSServerSatelliteCapsuleProjectVersion}` repository in the {ProjectServer}.
If you do not have Extended Update Support (EUS), synchronize `{RepoRHEL8ServerSatelliteCapsuleProjectVersion}` repository in the {ProjectServer}.
...
+
If you have Extended Update Support, ensure {SmartProxy} has access to `{RepoRHEL8EUSServerSatelliteMaintenanceProjectVersion}`.
If you do not have Extended Update Support, ensure {SmartProxy} has access to `{RepoRHEL8ServerSatelliteMaintenanceProjectVersion}.

. Upgrade {SmartProxy} to the next version:
+
[options="nowrap" subs="attributes"]
----
