:_mod-docs-content-type: PROCEDURE

[id="upgrading_{smart-proxy-context}_server_{context}"]
= Upgrading {SmartProxyServers}

[role="_abstract"]
ifdef::satellite[]
After upgrading your {ProjectServer} and synchronizing the required repositories, you can upgrade your {SmartProxyServers}.
endif::[]
ifndef::satellite[]
After you have upgraded your {ProjectServer}, you can start upgrading your {SmartProxyServers}.
endif::[]

ifndef::foreman-deb[]
[NOTE]
====
You can use the *{SmartProxy} Upgrade Playbook* job template to upgrade {SmartProxyServer}s remotely.
For more information about running remote jobs based on templates, see {ManagingHostsDocURL}configuring-and-setting-up-remote-jobs[Configuring and setting up remote jobs] in _{ManagingHostsDocTitle}_.
====
endif::[]

.Prerequisites
* The base system of the {SmartProxy} is registered to the newly upgraded {ProjectServer}.
ifdef::katello,orcharhino[]
* If you use content views to control updates to the base operating system of {SmartProxyServer}, ensure these content views are up-to-date with the new repositories.
endif::[]
ifdef::satellite[]
* Ensure that you synchronize the required repositories on {ProjectServer}.
For more information, see xref:synchronizing_the_new_repositories_{context}[].
* If the {SmartProxyServer} is registered to a content view, add the {SmartProxy} {ProjectVersion} repositories to the content view.
endif::[]
ifdef::katello,orcharhino,satellite[]
Publish and promote a new version of the content view.
For more information, see {ContentManagementDocURL}managing-content-views-and-related-configuration[Managing content views] in _{ContentManagementDocTitle}_.
endif::[]

.Procedure
ifdef::foreman-deb[]
[tabs]
=====
Debian 12 (Bookworm)::
+
--
:distribution-codename: bookworm
include::snip_upgrading-smartproxy-server.adoc[]
--
Ubuntu 22.04 (Jammy)::
+
--
:distribution-codename: jammy
include::snip_upgrading-smartproxy-server.adoc[]
--
=====
endif::[]
ifndef::foreman-deb[]
include::snip_upgrading-smartproxy-server.adoc[]
endif::[]
