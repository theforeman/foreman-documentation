...
* If you want to use an external PostgreSQL database, PostgreSQL must be installed on your host. For instructions, see {AdministeringDocURL}installing-postgresql_admin[Installing PostgreSQL] in _{AdministeringDocTitle}_.
endif::[]
...
ifdef::katello,satellite,orcharhino[]
. Optional: If you want to use an external PostgreSQL database in your {Project} deployment, configure your {ProjectServer}:
+
[options="nowrap" subs="+quotes,attributes"]
----
# {foreman-installer} \
...
----
+
To enable the Secure Sockets Layer (SSL) protocol for these external databases, append the following options:
+
[options="nowrap" subs="+quotes,attributes"]
----
