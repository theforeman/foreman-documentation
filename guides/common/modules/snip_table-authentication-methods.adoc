...
|xref:configuring-kerberos-sso-for-active-directory-users-in-{project-context}[]
|{FreeIPA}|Yes (Linux and Active Directory users)|Yes (Linux users only)|No|No
ifndef::satellite[]
|No
endif::[]
...
|xref:configuring-an-ldap-server-as-an-external-identity-provider-for-{project-context}[]
endif::[]
|====
endif::[]
