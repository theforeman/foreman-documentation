...
ifndef::satellite[]
| *Proxy tasks batch size for Ansible* | Empty | Number of tasks which should be sent to the {SmartProxy} in one request if {project_context}_tasks_proxy_batch_trigger is enabled.
If set, it overrides {project_context}_tasks_proxy_batch_size setting for Ansible jobs.
endif::[]
|====
