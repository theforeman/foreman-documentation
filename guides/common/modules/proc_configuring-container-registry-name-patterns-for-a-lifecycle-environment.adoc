...
[id="configuring-container-registry-name-patterns-for-a-lifecycle-environment"]
= Configuring container registry name patterns for a lifecycle environment

[role="_abstract"]
Set a registry name pattern for a lifecycle environment to override default container image names and to avoid naming conflicts when you pull images from {Project}.
