...
* For more information on ISS use cases and scenarios, see xref:inter-server-sync-scenarios_{context}[].
endif::[]

ifdef::satellite[]
There are two possible ISS configurations of {Project}, depending on how you deployed your infrastructure.
Configure your {Project} for ISS as appropriate for your scenario.
For more information, see {InstallingServerDisconnectedDocURL}inter-server-sync-scenarios_{project-context}[{ISS} scenarios] in _{InstallingServerDisconnectedDocTitle}_.
