[id="Extending_the_Discovery_Service_{context}"]
= Extending the Discovery service
[[Extending_the_Discovery_Image_{context}]]

You can extend the {Project} Discovery service with custom facts, software, or device drivers.
You can also provide a compressed archive file containing extra code for the service to use.
...
----
+
* The `autostart.d` directory contains scripts that are executed in POSIX order by the kernel image when it starts, but before the host is registered to {Project}.
...
----
. To inform the Discovery kernel image of the extensions it must use, place your zip files on your TFTP server with the Discovery images, and then update the `APPEND` line of the PXELinux template with the `fdi.zips` option where the paths are relative to the TFTP root.
