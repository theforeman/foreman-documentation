...
When {ProjectServer} is configured to use REX pull-based transport for remote execution jobs using the Script provider, {SmartProxyServer} sends notifications about new jobs to clients through MQTT.
