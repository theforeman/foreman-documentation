...
[role="_abstract"]
Browse the repository content of an organization in a web browser or with curl after you generate a debug certificate for that organization.

.Prerequisites
* You have created and downloaded an organization debug certificate.
...
. To use the organization debug certificate with a browser, you must first convert the X.509 certificate to a format your browser supports and then import the certificate.
For example, for Mozilla Firefox users:
...
----
.. In the Mozilla Firefox browser, navigate to *Edit* > *Settings* > *Privacy & Security*.
