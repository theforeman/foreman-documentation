...
.Procedure
. In the {ProjectWebUI}, navigate to *Content* > *Content Views* and choose a Content View.
. On the *Filters* tab, click *Create filter*.
. Enter a name.
. From the *Content type* list, choose a content type.
. Optional: Enter an arbitrary description.
. From the *Inclusion Type* list, choose either *Include* or *Exclude*.
...
. Click the *Affected repositories* tab to choose which specific repositories use this filter.
