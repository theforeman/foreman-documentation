...
[id="configuring-docker-to-trust-the-certificate-authority"]
= Configuring Docker to trust the certificate authority

[role="_abstract"]
Configure Docker to trust the {Project} certificate authority so that you can push and pull container images without disabling TLS verification.
...
----
# mkdir --parents /etc/docker/certs.d/_hostname.example.com_
----
. Copy the CA file to the directory:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# cp _My_Root_CA.pem_ /etc/docker/certs.d/_hostname.example.com_/ca.crt
----
