...
[id="adding-a-vmware-connection-to-{project-context}-server-by-using-web-ui"]
= Adding a VMware connection to {ProjectServer} by using {ProjectWebUI}

[role="_abstract"]
Add VMware vSphere as a compute resource to provision and manage hosts on VMware.
...
For more information, see xref:caching-of-compute-resources[].
