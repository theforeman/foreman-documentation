...
.  Assign the `foreman-proxy` user to the `named` manually. Normally, {foreman-installer ensures that the `foreman-proxy` user belongs to the `named` UNIX group, however, in this scenario {Project} do not manage users and group, therefore you need to assign the `foreman-proxy` user to the `named` manually.
+
[options="nowrap"]
----
