...
For instructions on obtaining and deploying the certificate securely, see {ManagingHostsDocURL}deploying-the-ca-certificate-on-a-host-manually[Deploying the CA certificate on a host manually] in _{ManagingHostsDocTitle}_.
endif::[]
ifdef::katello[]
...
* {ManagingHostsDocURL}enabling-custom-repositories-on-hosts[Enabling custom repositories on hosts in _{ManagingHostsDocTitle}_]
