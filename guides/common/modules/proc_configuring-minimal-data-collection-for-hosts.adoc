...
.Prerequisites
* Ensure existing data in Insights Inventory is deleted prior to switching a host to minimal reporting.
* You have selected an organization and location.       

.Procedure
. In the {ProjectWebUI}, navigate to *Insights* > *Inventory Upload*.
. Select the *Minimal data collection* setting from the dropdown menu under *Settings*. The settings cannot be changed for *Minimal data collection*.
+
[NOTE]
====
