...
[role="_abstract"]
The {ProjectName} content dashboard contains various widgets.
Use the dashboard to get a quick overview of the resources currently in use in {Project}.
...
| image:host-state-config-okay.png[] | *Hosts that had performed modifications without error* | Host that successfully performed modifications during the last reporting interval.
| image:host-state-config-error.png[] | *Hosts in error state* | Hosts on which an error was detected during the last reporting interval.
| image:host-state-config-report.png[] | *Good host reports in the last 35 minutes* | Hosts without error that did not perform any modifications in the last 35 minutes.
| image:host-state-config-pending.png[] | *Hosts that had pending changes* | Hosts on which some resources would be applied but Puppet was configured to run in the `noop` mode.
| image:host-state-config-outofsync.png[] | *Out of sync hosts* | Hosts that were not synchronized and the report was not received during the last reporting interval.
| image:host-state-config-noreport.png[] | *Hosts with no reports* | Hosts for which no reports were collected during the last reporting interval.
| image:host-state-config-noalert.png[] | *Hosts with alerts disabled* | Hosts which are not being monitored.
|====
+
...
In this case, each column represents several reports received from clients during 3 minutes.
...
*Host Collections*:: A list of all host collections in {Project} and their status, including the number of hosts in each host collection.
