[id="Updating-Disconnected-{project-context}-on-EL9-eus_{context}"]
= Updating a disconnected {ProjectServer} on {EL} 9 using EUS

You can update your disconnected {Project} on {RHEL} 9 by synchronizing the required Extended Update Support repositories on the connected {Project} and syncing the content to the disconnected {Project} using `reposync`.
...
* {RepoRHEL9BaseOS}
* {RepoRHEL9AppStream}
* {RepoRHEL9EUSServerSatelliteServerProjectVersion}
* {RepoRHEL9EUSServerSatelliteMaintenanceProjectVersion}
...
baseurl=_https://{foreman-example-com}_/pulp/content/_My_Organization_/Library/content/eus/rhel9/x86_64/satellite/{ProjectVersion}/os
enabled = 1
sslclientcert = /etc/pki/katello/certs/org-debug-cert.pem
sslclientkey = /etc/pki/katello/certs/org-debug-cert.pem
sslcacert = /etc/pki/katello/certs/katello-server-ca.crt
...
baseurl=_https://{foreman-example-com}_/pulp/content/_My_Organization_/Library/content/eus/rhel9/x86_64/sat-maintenance/{ProjectVersion}/os
enabled = 1
sslclientcert = /etc/pki/katello/certs/org-debug-cert.pem
sslclientkey = /etc/pki/katello/certs/org-debug-cert.pem
sslcacert = /etc/pki/katello/certs/katello-server-ca.crt
...
--repoid {RepoRHEL9EUSServerSatelliteMaintenanceProjectVersion} \
--repoid {RepoRHEL9EUSServerSatelliteServerProjectVersion} \
-n \
-p ~/{Project}-repos
----
+
...
baseurl=file:///root/{Project}-repos/{RepoRHEL9EUSServerSatelliteServerProjectVersion}
enabled = 1

[{RepoRHEL9EUSServerSatelliteMaintenanceProjectVersion}]
name={ProjectName} Maintenance 6 for RHEL 9 Server RPMs x86_64
baseurl=file:///root/{Project}-repos/{RepoRHEL9EUSServerSatelliteMaintenanceProjectVersion}
enabled = 1
----
