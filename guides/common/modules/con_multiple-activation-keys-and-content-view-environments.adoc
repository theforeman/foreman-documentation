...
[role="_abstract"]
Register hosts with multiple activation keys to combine settings from different keys, such as system purpose, repository overrides, and content view environments.
When conflicts occur, the last specified activation key takes precedence for conflicting settings.
