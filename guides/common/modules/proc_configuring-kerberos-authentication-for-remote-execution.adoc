...
.Prerequisites
* {ProjectServer} is enrolled on the Kerberos server.
* The {Project} target host is enrolled on the Kerberos server.
* A Kerberos user account for remote execution is configured and initialized.
* The `foreman-proxy` user on {Project} has a valid Kerberos ticket-granting ticket (TGT).
