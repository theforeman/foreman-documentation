...
[role="_abstract"]
Your system must meet the following general requirements before you can install {ProductName}.
...
ifdef::foremanctl[]
Do not use an existing system because `{foremanctl-installer}` affects the configuration of several components.
endif::[]
ifndef::foremanctl[]
