...
[role="_abstract"]
System requirements for {ProductName} include minimum CPU, RAM, storage, hostname format, SELinux configuration, and time synchronization to ensure proper installation and operation.
...
ifdef::containerized[]
Do not use an existing system because `{foremanctl}` affects the configuration of several components.
endif::[]
ifndef::containerized[]
Do not use an existing system because `{foreman-installer}` affects the configuration of several components.
endif::[]
...
For more information, see {RHELDocsBaseURL}10/html/configuring_time_synchronization/index[_{RHEL}{nbsp}10 Configuring time synchronization_].
endif::[]
...
For more information, see {PlanningDocURL}{ISS-id}-deployment-scenarios[{ISS} deployment scenarios in _{PlanningDocTitle}_].
endif::[]
...
ifndef::foreman-deb[]
ifdef::containerized[]
For more information about configuring system locale in {EL}, see {RHELDocsBaseURL}10/html/interactively_installing_rhel_from_installation_media/changing-basic-environment-settings#configuring-the-system-locale[Configuring the system locale] in _{RHEL}{nbsp}10 Interactively installing RHEL from installation media_.
endif::[]
ifndef::containerized[]
...
ifdef::containerized[]
* If you use an external identity provider in your deployment, ensure that the provider did not create the `{apache-user}` user account on the system.
