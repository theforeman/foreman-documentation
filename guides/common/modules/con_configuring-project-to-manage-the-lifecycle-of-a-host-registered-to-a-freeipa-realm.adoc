...
----
. On the {ProjectServer}, if you are configuring it for realm support, move the `/root/freeipa.keytab` file to the `/etc/foreman-proxy` directory:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# mv /root/freeipa.keytab /etc/foreman-proxy
----
. On both the {ProjectServer} and the {SmartProxyServer}s, set ownership to the `foreman-proxy` user and group:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
