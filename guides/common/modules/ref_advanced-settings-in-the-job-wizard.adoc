...
+
If you set the concurrency level to two or higher, remote execution jobs on hosts are executed concurrently.
For example, you can patch multiple hosts at the same time.
+
...
+
A high number of concurrent tasks may overload {SmartProxies}.
If you have many hosts registered to {SmartProxies}, batching prevents overloading your {SmartProxies}.
ifdef::satellite[]
...
You can control the number of tasks your {SmartProxy} receives at once from starting a remote execution job by setting the batch size.
ifdef::satellite[]
