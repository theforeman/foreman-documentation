...
[role="_abstract"]
Open the required network ports to enable communication between {Project} components across your infrastructure, including firewall configurations for both host-based and network-based firewalls.
...
ifndef::katello,satellite,orcharhino[--add-service=foreman-proxy \]
--add-service=http \
--add-service=https
----
