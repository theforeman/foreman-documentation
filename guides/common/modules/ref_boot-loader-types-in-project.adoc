...
[role="_abstract"]
You can use multiple types of PXE boot loaders during network provisioning, depending on firmware type, protocol, and boot environment.
...
| Grub2 UEFI HTTP
| `grub__My_Arch__.efi` (via HTTP)
| `grubx64.efi`

| Grub2 UEFI HTTPS
| `grub__My_Arch__.efi` (via HTTPS)
| `grubx64.efi`

| Grub2 UEFI HTTPS SecureBoot
| `shim__My_Arch__.efi` (via HTTPS)
| `shimx64.efi`
