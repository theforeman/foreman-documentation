...
[role="_abstract"]
Before you can deploy pre-built cloud instances or virtual machines, ensure that DNS and TFTP options are correctly configured so that hosts across multiple subnets or domains can successfully locate and retrieve the image installation content.

The requirements differ depending on the boot configuration method:
