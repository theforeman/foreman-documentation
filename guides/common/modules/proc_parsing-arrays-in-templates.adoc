...
[id="parsing-arrays-in-templates"]
...
.Procedure
. To retrieve the NIC of a host, in this example, using the `@host.interfaces` variable returns class values that you can then use to find methods to parse the array.
+
Example input:
+
----
