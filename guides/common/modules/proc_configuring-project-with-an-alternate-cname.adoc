...
----
# {foremanctl-installer} --certificate-cname _My_alternate_fqdn_
----
endif::[]
ifndef::foremanctl[]
...
[options="nowrap" subs="+quotes,attributes"]
----
# {foreman-installer} --certs-cname _My_alternate_fqdn_ --certs-update-server
----
. If you have not installed {Project}, you can add the `--certs-cname _My_alternate_fqdn_` option to the `{foreman-installer}` command to install {Project} with an alternate CNAME.
endif::[]
