...
* When Katello plugin is installed, the installer registers the host to {Project} using {Team} Subscription Manager.
* When Katello plugin is installed, management tools such as `katello-agent` and `puppet` are installed.
endif::[]
ifdef::satellite[]
* The installer registers the host to {Project} using {Team} Subscription Manager.
