...
[role="_abstract"]
Your system must meet the following storage requirements before you can install {ProjectServer}.
...
+
.Minimum storage requirements for a {ProjectServer} installation
...
ifndef::containerized[|/opt/puppetlabs | 500 MB | Not Applicable]
...
Because disk usage grows over time, monitor these partitions and expand them as required.
To make expanding storage easier, use Logical Volume Manager (LVM) for these volumes.
...
ifdef::containerized[]
nfs.example.com:/nfsshare  /var/lib/pulp  nfs  context="unconfined_u:object_r:var_lib_t:s0"  1 2
endif::[]
----
+
...
ifdef::containerized[]
* {RHELDocsBaseURL}10/html/managing_storage_devices/disk-partitions[Disk partitions in _{RHEL}{nbsp}10 Managing storage devices_]
endif::[]
ifndef::containerized[]
