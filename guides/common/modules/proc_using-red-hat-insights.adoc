...
Starting from {Project} {ProductVersion}, you can omit host reporting from _rh-cloud_ and _insight-client_ reports by setting the `host_registration_insights` parameter to *False*.
...
In the case where the host is already reported while setting the parameter to false, it can still be removed by the aging process from the cloud.
