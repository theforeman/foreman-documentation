...
.Prerequisites
* You have the `{kubevirt-command}` command-line tool installed.
...
----
.. Add the `kubevirt.io:edit` role to the service account:
+
[options="nowrap" subs="+quotes,attributes"]
----
$ {kubevirt-command} create rolebinding __My_Service_Account__-kubevirt-edit --clusterrole=kubevirt.io:edit --serviceaccount=__My_Namespace__:__My_Service_Account__
----
.. Add the `view` role to the service account:
+
[options="nowrap" subs="+quotes,attributes"]
----
$ {kubevirt-command} create rolebinding __My_Service_Account__-view --clusterrole=view --serviceaccount=__My_Namespace__:__My_Service_Account__
----
...
ifdef::satellite[]
* link:https://docs.redhat.com/en/products/openshift_container_platform/latest/html/cli_tools/openshift-cli-oc[OpenShift CLI (oc)]
endif::[]
ifndef::satellite[]
