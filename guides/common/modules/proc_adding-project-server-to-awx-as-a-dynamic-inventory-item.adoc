...
[id="adding-{project-context}-server-to-{awx-id}-as-a-dynamic-inventory-item"]
...
* For non-admin users, the `Ansible Tower Inventory Reader` role is assigned to your {ProjectServer} user.
...
* Your {ProjectServer} and {awx} are hosted on the same network or subnet.
