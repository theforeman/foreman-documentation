...
Your system must meet the following requirements for a successful installation and operation.
...
For more information, see {IntegratingProvisioningInfrastructureServicesDocURL}configuring-for-uefi-http-boot-provisioning-in-an-ipv6-network_{context}[Configuring {Project} for UEFI HTTP boot provisioning in an IPv6 network] in _{IntegratingProvisioningInfrastructureServicesDocTitle}_.
endif::[]
...
ifndef::containerized[]
* xref:adding-a-default-http-proxy-by-using-web-ui[]
endif::[]
