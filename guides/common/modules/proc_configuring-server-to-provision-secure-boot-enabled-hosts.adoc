[id="configuring-{project-context}-server-to-provision-{client-os-context}-on-Secure-Boot-enabled-hosts"]
= Configuring {ProjectServer} to provision {client-os} on Secure Boot enabled hosts
...
* Ensure that `ar` and `xz` are installed on your {ProjectServer}.
endif::[]
ifeval::["{client-os}" == "Ubuntu"]
* Ensure that `ar`, `xz`, and `zstd` are installed on your {ProjectServer}.
endif::[]
ifeval::["{client-pkg-ext}" == "rpm"]
* Ensure that `cpio` is installed on your {ProjectServer}.
endif::[]

.Procedure
. On your {ProjectServer}, configure the directory to store the shim and GRUB2 binaries required for provisioning Secure Boot enabled hosts:
+
...
[options="nowrap" subs="+quotes,verbatim,attributes"]
----
# BOOTLOADER_PATH="_My_Bootloader_Directory_/pxegrub2/_My_Operating_System_In_Lowercase_/_My_Operating_System_Version_/_My_Architecture_"
----
+
* To use one and the same shim and GRUB2 binaries for all versions of the operating system of your target host, replace _My_Operating_System_Version_ with `default`.
* If you require specific versions of the shim and GRUB2 binaries for the version of the operating system of your target host, use the *Major* and *Minor* version of the operating system separated by a dot (`.`) as _My_Operating_System_Version_.
If no *Minor* version is set, then use the *Major* version as _My_Operating_System_Version_.
+
...
└── pxegrub2
    ├── _My_Operating_System_In_Lowercase_
        └── _My_Operating_System_Version_
            └── _My_Architecture_
