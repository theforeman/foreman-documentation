...
[id="importing-a-content-view-version"]
= Importing a content view version

[role="_abstract"]
You can import an exported content view version to recreate the same content in an organization on a downstream {ProjectServer}.
...
ifdef::satellite[]
For more information, see {InstallingServerDisconnectedDocURL}configuring-project-server-to-synchronize-content-through-exports-by-using-web-ui[Configuring {ProjectServer} to synchronize content through exports by using {ProjectWebUI}] in _{InstallingServerDisconnectedDocTitle}_.
endif::[]
ifndef::satellite[]
...
* If there are any Red{nbsp}Hat repositories in the exported content, the subscription manifest of the importing organization must contain subscriptions for the products contained within the export.
endif::[]
* The user importing the content view version has the _Content Importer_ role.
