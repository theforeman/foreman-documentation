[id="installing-and-configuring-openvox-agent-on-a-host-during-registration"]
= Configuring OpenVox agenton a host during registration

You can install and configure the OpenVox agent on the host during registration.
...
ifndef::orcharhino[]
+
Until the https://bugzilla.redhat.com/show_bug.cgi?id=2177730[BZ2177730] is resolved, you must use host parameters to specify the Puppet agent configuration even in integrated setups where the OpenVox server is a {SmartProxyServer}.
endif::[]
ifdef::katello,orcharhino,satellite[]
