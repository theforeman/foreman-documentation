[id="configuring-openvox-agent-on-a-host-during-provisioning_{context}"]
= Configuring OpenVox agent on a host during provisioning

You can install and configure the OpenVox agent on a host during the provisioning process.
...
For more information, see xref:installing-openvox-server-on-{project-context}-server-and-{smart-proxy-context}-servers[].
* You created a product and repository containing the OpenVox Puppet agent and synchronized the repository to {Project}.
For more information, see {ContentManagementDocURL}Importing_Content_content-management[Importing content] in _{ContentManagementDocTitle}_.
* You created an activation key that enables the OpenVox agent repository for hosts.
For more information, see {ContentManagementDocURL}Managing_Activation_Keys_content-management[Managing activation keys] in _{ContentManagementDocTitle}_.
endif::[]
...
** Add a host parameter named `puppet_server`, select the *string* type, and set the value to the hostname of your Puppet server, such as `openvox.example.com`.
