...
ifdef::satellite[]
If the parameter is set to `true`, the host is included in the Red{nbsp}Hat Lightspeed Inventory report uploaded to the {RHCloud}. 
If the parameter is set to `false`, the host is excluded from the Red{nbsp}Hat Lightspeed Inventory report upload.
To upload inventory data without registering the host with the Red{nbsp}Hat Lightspeed client, set the `host_registration_insights` parameter to `false` and set the `host_registration_insights_inventory` to `true`.
endif::[]
