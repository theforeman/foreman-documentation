...
[id="installing-smart-proxy-server"]
= Installing {SmartProxyServer}

[role="_abstract"]
Install the required packages and configure {SmartProxyServer} to extend {Project} services to a remote network.
...
# {package-upgrade}
----
. Install `{smartproxy-installer-package}`:
+
[options="nowrap" subs="+quotes,attributes"]
----
