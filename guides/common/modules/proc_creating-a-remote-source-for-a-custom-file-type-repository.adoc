...
[id="creating-a-remote-source-for-a-custom-file-type-repository"]
= Creating a remote source for a {customfiletype} repository

[role="_abstract"]
You can prepare a directory on a remote HTTP server as a Pulp source and synchronize files into a {customfiletype} repository on {ProjectServer}.

Configure a directory on a remote server as the repository source.
