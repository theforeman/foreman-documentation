...
def main():
    """Create or reuse an organization and create lifecycle environments."""
    ...
    # Retrieve all lifecycle environments in the organization.
    envs = get_json(
        f"\{KATELLO_API}/organizations/\{org_id}/environments/"
    )

    # Map environment IDs to names and find the Library environment ID.
    prior_env_id = 0
    env_list = {}
