...
[role="_abstract"]
With {keycloak} configured as an authentication source for {Project}, users defined in a {keycloak} realm can log in to {ProjectServer}.
The available login methods depend on how you configured integration between {keycloak} and {Project}.
