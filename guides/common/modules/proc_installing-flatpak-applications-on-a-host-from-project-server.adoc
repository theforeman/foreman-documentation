...
[id="installing-flatpak-applications-on-a-host-from-{project-context}-server"]
= Installing Flatpak applications on a host from {ProjectServer}

[role="_abstract"]
Connect your host to the Flatpak remote on {ProjectServer}, authenticate to the container registry, and install Flatpak applications published through your content views.
...
* Flatpak application repositories are published through content views.
...
* Podman is configured to trust the certificate authority of {ProjectServer}.
...
{Project} generates a token automatically when you run `podman login`.
