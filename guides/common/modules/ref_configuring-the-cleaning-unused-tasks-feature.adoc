...
ifdef::containerized[]
* To configure the time at which {Project} runs the cron job, set the systemd timer to the time you want in OnCalendar format.
For example, to schedule the cron job to run every day at 15:00, run the following commands:
...
[NOTE]
====
The timer uses `RandomizedDelaySec=120`, so the cleanup may start up to 2 minutes after the configured time.
====
