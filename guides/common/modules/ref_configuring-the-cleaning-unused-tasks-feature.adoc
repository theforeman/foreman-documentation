...
. Edit the timer's schedule with a drop-in override:
+
[options="nowrap" subs="+quotes,attributes"]
----
...
ifdef::containerized[]
* To configure the period after which {Project} deletes the tasks, use `foreman-rake` task parameters (`TASK_SEARCH`, `AFTER`, `STATES`, etc.) or the web UI.
endif::[]
