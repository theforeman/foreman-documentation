...
[role="_abstract"]
You can create a job template from the {ProjectWebUI} to define a reusable script or Ansible task that you can run on hosts via remote execution.
...
ifdef::satellite[]
For more information on Ansible check mode, see {RHDocsBaseURL}red_hat_ansible_automation_platform/latest/configure-about_configuration_as_code#about-configuration-as-code__check-mode[Check mode and idempotency] in _Red{nbsp}Hat Ansible Automation Platform documentation_.
endif::[]
