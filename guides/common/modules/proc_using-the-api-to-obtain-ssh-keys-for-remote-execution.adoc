...
[role="_abstract"]
You can obtain the {Project} SSH key from {SmartProxy} and add it to the `authorized_keys` on the host so that remote execution jobs can run on the host via SSH.
