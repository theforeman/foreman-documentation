...
Configure {Team} Subscription Manager to use {ProjectServer} for registration and specify the activation key when running the `subscription-manager register` command.
...
----
+
This RPM installs the necessary certificates for accessing repositories on {ProjectServer} and configures {Team} Subscription Manager to use the server's URL.
