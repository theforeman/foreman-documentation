...
ifdef::satellite[]
.Prerequisites
* To sync {Project} from Red{nbsp}Hat Private Automation Hub, you need an authentication token.
To obtain it, go to https://console.redhat.com/ansible/automation-hub/token[Connect to Hub], and click *Load token*.
* To sync {Project} from `console.redhat.com`, you need an authentication token and SSO URL.
...
.. To sync {Project} from `console.redhat.com`, enter your token in the *Auth Token* field and enter your SSO URL in the *Auth URL* field.
