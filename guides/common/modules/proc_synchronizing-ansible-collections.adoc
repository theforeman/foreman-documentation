...
[role="_abstract"]
Synchronize Ansible Collections from Automation Hub, Ansible Galaxy, or other {Project} instances to distribute automation content to managed hosts.
Synchronized collections enable consistent Ansible automation across your infrastructure.

.Prerequisites
* To sync Ansible collections from Red{nbsp}Hat Private Automation Hub, you need to provide {Project} with an authentication token.
To obtain it, go to https://console.redhat.com/ansible/automation-hub/token[Connect to Hub], and click *Load token*.
* To sync Ansible collections from `console.redhat.com`, you need an authentication token and SSO URL.
To obtain them, go to https://console.redhat.com/ansible/automation-hub/token[Connect to Hub].
Click *Load token* to obtain the token.
Locate the *SSO URL* section to obtain the SSO URL.
