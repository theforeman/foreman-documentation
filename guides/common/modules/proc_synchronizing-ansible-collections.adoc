...
.Prerequisites
* To sync Ansible collections from Red Hat Private Automation Hub, you have an *authentication token*.
* To sync Ansible collections from `console.redhat.com`, you have an *authentication token* and the *SSO URL*.
...
+
The system automatically populates the *Label* field based on the name.
...
+
If you do not specify a list of collections, the system synchronizes all Ansible collections from the endpoint.
+
[source, yaml]
----
---
...
** Navigate to https://console.redhat.com/ansible/automation-hub/token[Connect to Hub] > click *Load token*.
** Enter the token in the *Auth Token* field.
* To sync Ansible collections from `console.redhat.com`:
** Navigate to https://console.redhat.com/ansible/automation-hub/token[Connect to Hub] > click *Load token*.
** Enter the authentication token in the *Auth Token* field.
** Copy the SSO URL from the SSO URL section.
** Enter the SSO URL in the *Auth URL* field.
