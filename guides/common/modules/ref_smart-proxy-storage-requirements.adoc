...
[id="{smart-proxy-context}-storage-requirements"]
= {SmartProxy} storage requirements

[role="_abstract"]
Ensure sufficient storage is allocated to the required directories for successful installation and operation of {SmartProxyServer}.
Note that the values can vary according to individual environments.
