...
You can deploy a `virt-who` configuration on a hypervisor.
Global values apply only to this hypervisor.

You can deploy a vCenter or Hyper-V `virt-who` configuration on {SmartProxyServer}.
...
* You have updated {oVirt} to the latest version so that the minimum `virt-who` version is available. `virt-who` is available by default on {oVirt}, but you cannot update `virt-who` by using the `rhel-7-server-rhvh-4-rpms` repository.
* You have created a read-only virt-who user on the hypervisor.
...
. Copy the script to the hypervisor by running the following command:
+
...
. After the deployment is complete, delete the script by running the following command:
+
[options="nowrap" subs="+quotes"]
----
