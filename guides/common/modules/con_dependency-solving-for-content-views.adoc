...
* When incrementally adding a security errata to a content view, enabling dependency solving causes significant delays to content view publication without major benefits.
...
For more information, see xref:dependency-resolution-limitations-for-incremental-errata-updates[].
====
