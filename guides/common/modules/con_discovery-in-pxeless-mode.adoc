:_mod-docs-content-type: CONCEPT

[id="discovery-in-pxeless-mode"]
= Discovery in PXE-less mode

[role="_abstract"]
In environments without DHCP and TFTP, you can use PXE-less Discovery by using a Discovery ISO.
Discovered hosts queued for installation reload the kernel with an operating system installer without rebooting.

ifdef::satellite[]
:FeatureName: Discovery `kexec`
include::snip_technology-preview.adoc[]

You might experience the following problems:

* The console might freeze during the process.
* On some hardware, you might experience graphical hardware problems.
endif::[]

.Discovery workflow in PXE-less mode
ifdef::satellite[]
image::discovery-pxeless-mode-satellite.png[Discovery workflow in PXE-less mode]
endif::[]
ifdef::orcharhino[]
image::discovery-pxeless-mode-orcharhino.svg[Discovery workflow in PXE-less mode]
endif::[]
ifndef::satellite,orcharhino[]
image::discovery-pxeless-mode.svg[Discovery workflow in PXE-less mode]
endif::[]
