...
For more information, see xref:configuring-{smart-proxy-context}-server-with-a-default-ssl-certificate[].
* If you use a custom SSL certificate, you must also configure each {SmartProxyServer} with a distinct custom SSL certificate.
For more information, see xref:configuring-{smart-proxy-context}-server-with-a-custom-ssl-certificate[].
