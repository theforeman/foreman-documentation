...
omapi-port 7911;
omapi-key _key_name_;
----
+
The settings specified in the example include the following:
+
`key "_key_name_"`:: Defines the key, its algorithm and encrypted password.
...
`omapi-key _key_name_`:: Defines the name of the key the OMAPI interface uses.
