...
[id="using-vmware-cloud-init-and-userdata-templates-for-provisioning"]
...
.Prerequisites
* The templates are associated with your operating system.
For more information, see xref:associating-the-userdata-and-cloud-init-templates-with-the-operating-system[].
* An image is prepared.
For more information, see xref:preparing-an-image-to-use-the-cloud-init-template[].
* Port and firewall settings are configured to open any necessary connections.
...
ifndef::orcharhino[]
For more information, see {PlanningDocURL}{project-context}-port-and-firewall-requirements[{ProjectServer} port and firewall requirements] in _{PlanningDocTitle}_ and {PlanningDocURL}{smart-proxy-context}-port-and-firewall-requirements[{SmartProxy} port and firewall requirements] in _{PlanningDocTitle}_.
endif::[]
+
include::snip_prerequisite-configured-smart-proxy-registration-provisioning.adoc[]
* You have backups of the following configuration files:
...
.Next steps
* xref:adding-vmware-images-to-{project-context}-server-by-using-cli[]
