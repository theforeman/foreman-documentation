...
[id="restricting-a-{customrepoid}-to-a-specific-operating-system-version-or-architecture"]
= Restricting a {customrepo} to a specific operating system version or architecture

[role="_abstract"]
You restrict access to a {customrepo} so only matching hosts consume the repository from {Project}.
For example, expose the repository only to {client-os}{nbsp}{client-os-major} hosts.
