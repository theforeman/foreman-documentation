...
* You have a copy of the GPG key used to sign the RPM content that you want to use and manage in {Project}.
...
* You have extracted the GPG public key fingerprints from a signed `Release` file for Deb content.
...
. Paste the GPG key into the *Content* field.
Alternatively, click *Choose file* and upload the public key.
+
If your {customrepo} contains content signed by multiple GPG keys, enter all required GPG keys in the *Content* field with new lines between each key, for example:
+
----
