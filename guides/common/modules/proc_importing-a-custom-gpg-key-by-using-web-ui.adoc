...
[role="_abstract"]
Import GPG keys into {Project} so mirrors and hosts can trust signatures on signed {customcontent}, reducing the risk of tampered packages in your software pipeline.

ifdef::satellite[]
[NOTE]
====
Red{nbsp}Hat content is already configured with the appropriate GPG key; GPG key management for Red{nbsp}Hat repositories is not supported.
====
endif::[]
