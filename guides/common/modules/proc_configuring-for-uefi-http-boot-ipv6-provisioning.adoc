...
. Since clients that are booting from network cannot be identified by a remote IPv6 address due to unmanaged DHCPv6 service, you must disable provisioning tokens. To disable provisioning tokens, navigate to *Administer* > *Settings* > *Provisioning* and set the *Token duration* setting to 0.
