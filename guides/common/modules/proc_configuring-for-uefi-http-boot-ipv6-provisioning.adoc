...
. {Project} cannot identify clients that are booting from the network by a remote IPv6 address because of unmanaged DHCPv6 service, you must enable provisioning tokens. Provisioning tokens are enabled by default. To configure provisioning tokens duration, navigate to *Administer* > *Settings* > *Provisioning* and change the *Token duration* setting. Note that *Token duration* set to *0* disables the tokens.
