...
Using structured APT content has the following benefits:

Comprehensibility::
Repositories in {Project} use the same structure as the upstream repository.
They do not use the special `default/all` publication concept which does not exist outside of {Project}.

Performance::
Publishing repository metadata will take roughly half the time, compared to publishing without structured APT content.

However, using structured APT content has the following limitations:
--
