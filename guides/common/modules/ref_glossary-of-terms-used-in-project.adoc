...
Catalogs are compiled by a Puppet server from Puppet Manifests and data from OpenVox agents.
...
[[Puppet_server]]
Puppet server:: A {SmartProxyServer} component that provides a Puppet catalog to hosts for execution by the OpenVox agent.
