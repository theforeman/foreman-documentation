...
[id="glossary-of-terms-used-in-{project-context}"]
...
+
Activation keys can be associated with content view environments, each of which combines a content view with a lifecycle environment.
When the *Allow multiple content views* setting is enabled, an activation key can be associated with multiple content view environments.
Otherwise, you can associate an activation key with at most one content view environment.
...
+
image::activation_key.svg[Activation key]
endif::[]
...
For more information, see {ContentManagementDocURL}promoting-a-content-view-by-using-web-ui[Promoting a content view] in _{ContentManagementDocTitle}_.
endif::[]
...
{Project} provides several predefined roles.
