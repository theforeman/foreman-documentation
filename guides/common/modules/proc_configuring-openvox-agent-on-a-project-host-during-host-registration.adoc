...
[id="configuring-openvox-agent-on-a-project-host-during-host-registration"]
= Configuring OpenVox agent on a {Project} host during host registration

[role="_abstract"]
If you prefer to install and configure the OpenVox agent on a host during the registration process, you can enable the OpenVox agent with host parameters in global parameters or a host group.
