...
[id="best-practices-for-products-and-repositories"]
= Best practices for products and repositories

[role="_abstract"]
Follow these recommendations when you organize products and repositories in {Project} to simplify content views and avoid synchronization issues.
...
+
For more information, see xref:managing-custom-file-type-content-in-{project-context}[].
endif::[]
...
For more information, see xref:repository-definitions-for-oracle-linux-on-public-yum-servers[] and xref:repository-definitions-for-oracle-linux-on-uln[].
endif::[]
