...
[role="_abstract"]
Add Deb packages to a content view version as an incremental update when you need to make specific packages installable on Debian or Ubuntu hosts.

To add errata to a content view version instead, see xref:adding-errata-to-an-incremental-content-view-by-using-web-ui[].
