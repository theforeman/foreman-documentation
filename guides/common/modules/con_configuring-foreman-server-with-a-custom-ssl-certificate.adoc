...
* SSL certificate and Certificate Authority (CA) bundle files must contain only `-----BEGIN CERTIFICATE-----` and `-----END CERTIFICATE-----` blocks.
Blank lines between the blocks are allowed.
The files must not contain comments or other text outside those blocks.
...
* If you use a certificate signed by an intermediate CA, you must provide the full chain of certificates.
Your certificate must start with the Root CA, contain one or more intermediate CAs, and end with your server certificate.
