...
ifdef::katello[]
xref:adding-custom-deb-repositories-by-using-web-ui[] or xref:adding-custom-rpm-repositories-by-using-web-ui[].
endif::[]
ifdef::orcharhino[]
ifdef::debian,ubuntu[]
xref:adding-custom-deb-repositories-by-using-web-ui[].
endif::[]
