...
----
. In the {ProjectWebUI}, navigate to *Configure*{nbsp}>{nbsp}*Environments* and click *Import environment from*.
...
In the {ProjectWebUI}, navigate to *Infrastructure*{nbsp}>{nbsp}*{SmartProxies}* and find the {Project} or {SmartProxyServer} you specified with the `--server` option.
...
. In the {ProjectWebUI}, navigate to *Hosts*{nbsp}>{nbsp}*All hosts* and ensure that the host is connected to the correct host group.
