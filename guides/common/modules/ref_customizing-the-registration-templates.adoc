...
* The `host_registration_remote_execution` parameter is used in the `remote_execution_script_keys` snippet, the default value is `true`.
...
* The `remote_execution_script_keys`, `remote_execution_script_user`, `remote_execution_create_user` and `remote_execution_effective_user_method` parameters are used in the `remote_execution_script_keys`. For more details se the details of the snippet.
...
* The `remote_execution_script_keys` snippet deploys Script keys to the host only when the `host_registration_remote_execution` parameter is `true`.
...
|`setup_remote_execution`
|Override the value of `host_registration_remote_execution` global parameter for the registered host and deploy Script keys for remote execution.
