:_mod-docs-content-type: CONCEPT

[id="performing-additional-configuration-on-server_{context}"]
= Performing additional configuration on {ProjectServer}

[role="_abstract"]
You can enable additional features on {ProjectServer}, such as {Insights} integration, pull-based transport for remote execution, or the usage of HTTP proxies.
While you can configure these features after the initial setup, you can also configure many of them during the installation by using the corresponding ifdef::foremanctl[`{foremanctl-installer}`]ifndef::foremanctl[`{foreman-installer}`] options.

