...
[id="collecting-sosreport-data_{context}"]
= Collecting diagnostic data with sos report

[role="_abstract"]
You can collect diagnostic data from your containerized {ProjectServer} by using the `sos report` command and attach the resulting archive to a {Team} support case.
The container-aware `sos` plugins automatically collect `journald` logs, container state, service configuration, and {SmartProxy} data.
