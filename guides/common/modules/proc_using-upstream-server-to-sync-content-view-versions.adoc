...
[role="_abstract"]
Use an upstream {ProjectServer} to curate and synchronize content view versions before exporting to downstream servers in isolated networks.
This approach enables controlled content management and lifecycle progression for disconnected environments.
...
+
If you want to sync custom content, first xref:creating-a-custom-product-by-using-web-ui[create a custom product] and then xref:repository-synchronization[synchronize repositories].
