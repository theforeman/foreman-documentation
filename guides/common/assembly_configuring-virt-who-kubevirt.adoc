...
:kubevirt-hypervisor:
:a-target: a VM running on {KubeVirt} itself
:the-target: the VM running on {KubeVirt} itself
:target-name: clientvm.example.com
include::modules/proc-deploying-virt-who-on-kubevirt-vm.adoc[leveloffset=+1]
:!kubevirt-hypervisor:
