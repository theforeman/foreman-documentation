...
In Satellite Server {ProductVersion}, OSTree management tools are enabled by default. If you ever have a reason to enable the tool, enter the following command:
...
* In the Satellite web UI, navigate to *Content* > *Sync Status* and expand, for example, *Red{nbsp}Hat Enterprise Linux Atomic Host*.
