...
2. Stop the `satellite-maintain` services on the {Project} host:
+
----
...
11. Start the `satellite-maintain` services on the {Project} host:
+
----
# satellite-maintain service start
----

{ProjectServer} now uses the NFS share to store content. Run a content synchronization (see <<Importing_Red_Hat_Content-Synchronizing_Content>>) to ensure the NFS share works as expected.
