...
. From the *Content View* list, select a Content View to use. If you want to use this activation key to register hosts, the Content View must contain the {Project} Tools repository because it is required to install the `katello-agent`.
. Click *Save*.
...
----
+
. Override the default auto-enable status for the {ProjectName} Tools {ProductVersion} repository. The default status is set to disabled. To enable, enter the following command:
+
...
. Download the consumer RPM for your {ProjectServer}. This is located in the `pub` directory on the host's web server. For example, for a {ProjectServer} with the host name `satellite.example.com`, enter the following command on the host to register:
+
[options="nowrap" subs="+quotes"]
----
...
----
+
The {ProjectName} Tools {ProductVersion} repository provides this package.
