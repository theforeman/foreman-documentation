...
To apply an erratum to a RHEL 8 host, you can run a remote execution job on Satellite Server or update the host. For more information about running remote execution jobs, see https://doc-stage.usersys.redhat.com/documentation/en-us/red_hat_satellite/{ProductVersion}/html-single/managing_hosts/#Running_Jobs_on_Hosts[Running Jobs on Hosts] in the  _Managing Hosts_ guide.
