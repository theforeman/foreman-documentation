...
To create a file type repository in a directory on the base system where {ProjectServer} is installed, see xref:Importing_Custom_Content-Creating_a_Custom_File_Type_Repository_Local_Directory[].
...
For more information see xref:Importing_Custom_Content-Creating_a_Custom_File_Type_Repository[]
. In the {Project} web UI, navigate to *Content* > *Products*.
. Select the name of a product.
. Click the *Repositories* tab and select *New Repository*.
. In the *Name* field, enter a name for the repository.
...
See xref:Importing_Custom_Content-Creating_a_Custom_File_Type_Repository[] for more information.
