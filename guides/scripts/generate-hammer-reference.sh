...
asmb_footer='include::common/modules/ref_hammer-option-details.adoc[leveloffset=+1]\n\nifndef::orcharhino,satellite[]\ninclude::common/ribbons.adoc[]\nendif::[]\n'
...
# FYI read strips leading spaces
    ...
    if [ -n "$skip_section" ]; then
        ...
        if echo -e "${line}" | grep -Eq '^-{1,2}[a-z\[]' - ; then # option line
            ...
            fixed_line=`echo -e "$fixed_line" | sed -E 's/(--\S+\s[A-Z\_]{3,})/\`\1\`/' | sed -E "s|([A-Z\\_]{3,})(\\\`)|xref:${details_file}#hammer-option-details-\\L\\1\\E\\[\\1\\]\\2|"`
