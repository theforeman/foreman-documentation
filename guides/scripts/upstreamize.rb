    ...
    begin
      ...
      File.open(temp_file, 'w') do |output|
        ...
        File.foreach(input_file).with_index do |orig_line, line_num|
          ...
          line.gsub!(/Red_Hat_Satellite/, '{context}')
          line.gsub!(/Satellite( |\u{00A0}|{nbsp})6.\d+/, '{ProjectXY}')
          line.gsub!(/Satellite( |\u{00A0}|{nbsp})6/, '{ProjectX}')
          line.gsub!(/(?:Red( |\u{00A0}|{nbsp})Hat )?Satellite( |\u{00A0}|{nbsp})Server/, '{ProjectServer}')
          line.gsub!(/(?:Satellite )?web UI/, '{ProjectWebUI}')
          ...
          line.gsub!(/satellite-maintain packages install/, '{project-package-install}')
          line.gsub!(/satellite-maintain packages remove/, '{project-package-remove}')
