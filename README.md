...
* `html`: builds HTML guides with all contexts (`foreman-el`, `foreman-deb`, `containerized-katello`, `containerized-orcharhino`, `katello`, `orcharhino`, and `satellite`)
...
Run `make vale` to run the CI-style changed-file check in Podman without installing Vale locally.
Use `make vale-all` to scan all guides.
